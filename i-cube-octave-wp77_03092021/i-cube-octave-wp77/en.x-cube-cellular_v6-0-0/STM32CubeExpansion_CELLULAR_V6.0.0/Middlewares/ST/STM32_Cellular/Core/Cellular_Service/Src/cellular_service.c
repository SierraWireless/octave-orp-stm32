...
static csint_urc_subscription_t cs_ctxt_urc_subscription = {
		.eps_network_registration = CELLULAR_FALSE, .gprs_network_registration =
				CELLULAR_FALSE, .cs_network_registration = CELLULAR_FALSE,
		.eps_location_info = CELLULAR_FALSE, .gprs_location_info =
				CELLULAR_FALSE, .cs_location_info = CELLULAR_FALSE,
		.signal_quality = CELLULAR_FALSE, .packet_domain_event = CELLULAR_FALSE,
		.ping_rsp = CELLULAR_FALSE, };
static csint_location_info_t cs_ctxt_eps_location_info = { .ci = 0, .lac = 0,
		.ci_updated = CELLULAR_FALSE, .lac_updated = CELLULAR_FALSE, };
static csint_location_info_t cs_ctxt_gprs_location_info = { .ci = 0, .lac = 0,
		.ci_updated = CELLULAR_FALSE, .lac_updated = CELLULAR_FALSE, };
static csint_location_info_t cs_ctxt_cs_location_info = { .ci = 0, .lac = 0,
		.ci_updated = CELLULAR_FALSE, .lac_updated = CELLULAR_FALSE, };
			...
			if (AT_open_channel(_Adapter_Handle) == ATSTATUS_OK) {
				...
				if (DATAPACK_writeStruct(&cmd_buf[0], (uint16_t) CSMT_NONE,
					...
					err = AT_sendcmd(_Adapter_Handle,
							(at_msg_t) SID_CS_POWER_ON, &cmd_buf[0],
							&rsp_buf[0]);
					if (err == ATSTATUS_OK) {
...
CS_Status_t CS_check_connection(void) {
	...
	NULL) == DATAPACK_OK) {
		at_status_t err;
		err = AT_sendcmd(_Adapter_Handle, (at_msg_t) SID_CS_CHECK_CNX,
				&cmd_buf[0], &rsp_buf[0]);
		if (err == ATSTATUS_OK) {
...
CS_Status_t CS_sim_select(CS_SimSlot_t simSelected) {
	...
	if (DATAPACK_writeStruct(&cmd_buf[0], (uint16_t) CSMT_SIM_SELECT,
			(uint16_t) sizeof(CS_SimSlot_t), (void*) &simSelected)
			== DATAPACK_OK) {
		at_status_t err;
	...
	if (retval == CELLULAR_ERROR) {
		PRINT_INFO(
				"<Cellular_Service> SIM selection need a modem restart to be applied")
	}
	return (retval);
}
...
int32_t CS_sim_generic_access(CS_sim_generic_access_t *sim_generic_access) {
	...
	(void) memset((void*) &sim_generic_access_data, 0,
			sizeof(sim_generic_access_data));
	...
	if ((sim_generic_access->p_cmd_str != NULL)
		...
		if (DATAPACK_writeStruct(&cmd_buf[0],
			...
			at_status_t err;
			err = AT_sendcmd(_Adapter_Handle,
					(at_msg_t) SID_CS_SIM_GENERIC_ACCESS, &cmd_buf[0],
					&rsp_buf[0]);
			if (err == ATSTATUS_OK) {
		...
		PRINT_ERR(
				"SIM generic access Buffer NULL, CMD size too big=%ld/%ld or RSP size too small=%ld/%ld)",
				sim_generic_access->cmd_str_size,
				CONFIG_MODEM_MAX_SIM_GENERIC_ACCESS_CMD_SIZE,
				sim_generic_access->rsp_str_size,
				CONFIG_MODEM_MIN_SIM_GENERIC_ACCESS_RSP_SIZE)
	...
	if (status == CELLULAR_ERROR) {
		...
		PRINT_INFO("Size of response received = %ld bytes",
				sim_generic_access_data.bytes_received)
 ...
 * @retval CS_Status_t
 */
CS_Status_t CS_init_modem(CS_ModemInit_t init, CS_Bool_t reset,
	...
	(void) memcpy((void*) &modemInit_struct.pincode.pincode[0],
			(const CS_CHAR_t*) pin_code, strlen((const CRC_CHAR_t*) pin_code));

	if (DATAPACK_writeStruct(&cmd_buf[0], (uint16_t) CSMT_INITMODEM,
			(uint16_t) sizeof(csint_modemInit_t), (void*) &modemInit_struct)
			== DATAPACK_OK) {
		at_status_t err;
...
CS_Status_t CS_register_net(CS_OperatorSelector_t *p_operator,
	...
	if (DATAPACK_writeStruct(&cmd_buf[0], (uint16_t) CSMT_OPERATORSELECT,
			(uint16_t) sizeof(CS_OperatorSelector_t), (void*) &cs_ctxt_operator)
			== DATAPACK_OK) {
		at_status_t err;
		...
		if (err == ATSTATUS_OK) {
			if (DATAPACK_readStruct(&rsp_buf[0],
					...
					(uint16_t) sizeof(CS_RegistrationStatus_t), p_reg_status)
					== DATAPACK_OK) {
 ...
 * @retval CS_Status_t
 */
CS_Status_t CS_subscribe_net_event(CS_UrcEvent_t event,
	...
	if (retval != CELLULAR_ERROR) {
		if (DATAPACK_writeStruct(&cmd_buf[0], (uint16_t) CSMT_URC_EVENT,
				(uint16_t) sizeof(CS_UrcEvent_t), (void*) &event)
				== DATAPACK_OK) {
			at_status_t err;
			err = AT_sendcmd(_Adapter_Handle,
					(at_msg_t) SID_CS_SUSBCRIBE_NET_EVENT, &cmd_buf[0],
					&rsp_buf[0]);
			if (err != ATSTATUS_OK) {
				retval = CELLULAR_ERROR;
			}
...
CS_Status_t CS_unsubscribe_net_event(CS_UrcEvent_t event) {
	...
	if (retval != CELLULAR_ERROR) {
		if (DATAPACK_writeStruct(&cmd_buf[0], (uint16_t) CSMT_URC_EVENT,
			...
			at_status_t err;
			err = AT_sendcmd(_Adapter_Handle,
					(at_msg_t) SID_CS_UNSUSBCRIBE_NET_EVENT, &cmd_buf[0],
					&rsp_buf[0]);
			if (err != ATSTATUS_OK) {
				retval = CELLULAR_ERROR;
			}
 ...
 * @retval CS_Status_t
 */
CS_Status_t CS_subscribe_sim_event(
	...
	NULL) == DATAPACK_OK) {
		...
		if (err == ATSTATUS_OK) {
			if (DATAPACK_readStruct(&rsp_buf[0], (uint16_t) CSMT_ATTACHSTATUS,
					(uint16_t) sizeof(CS_PSattach_t), p_attach)
					== DATAPACK_OK) {
...
CS_Status_t CS_activate_pdn(CS_PDN_conf_id_t cid) {
	...
	if (DATAPACK_writeStruct(&cmd_buf[0], (uint16_t) CSMT_ACTIVATE_PDN,
			(uint16_t) sizeof(CS_PDN_conf_id_t), (void*) &cid) == DATAPACK_OK) {
		at_status_t err;
...
CS_Status_t CS_deactivate_pdn(CS_PDN_conf_id_t cid) {
	...
	if (DATAPACK_writeStruct(&cmd_buf[0], (uint16_t) CSMT_DEACTIVATE_PDN,
			(uint16_t) sizeof(CS_PDN_conf_id_t), (void*) &cid) == DATAPACK_OK) {
		at_status_t err;
		err = AT_sendcmd(_Adapter_Handle, (at_msg_t) SID_CS_DEACTIVATE_PDN,
				&cmd_buf[0], &rsp_buf[0]);
		if (err == ATSTATUS_OK) {
 ...
 * @retval CS_Status_t
 */
CS_Status_t CS_define_pdn(CS_PDN_conf_id_t cid, const CS_CHAR_t *apn,
	...
	if ((cid < CS_PDN_USER_CONFIG_1) || (cid > CS_PDN_USER_CONFIG_5)) {
		PRINT_ERR(
				"<Cellular_Service> selected configuration id %d can not be set by user",
				cid)
 ...
 * @retval CS_Status_t
 */
CS_Status_t CS_get_dev_IP_address(CS_PDN_conf_id_t cid,
	...
	if (DATAPACK_writeStruct(&cmd_buf[0], (uint16_t) CSMT_GET_IP_ADDRESS,
		...
		if (err == ATSTATUS_OK) {
			...
			if (DATAPACK_readStruct(&rsp_buf[0], (uint16_t) CSMT_GET_IP_ADDRESS,
					(uint16_t) sizeof(csint_ip_addr_info_t), &ip_addr_info)
					== DATAPACK_OK) {
				...
				*ip_addr_type = ip_addr_info.ip_addr_type;
				...
				PRINT_DBG("<Cellular_Service> IP address = %s (type = %d)",
						(CS_CHAR_t* )ip_addr_info.ip_addr_value,
	...
	if (retval == CELLULAR_ERROR) {
		PRINT_ERR(
				"<Cellular_Service> error when getting IP address information")
	}
	return (retval);
}
 ...
 * @retval CS_Status_t
 */
CS_Status_t CS_subscribe_modem_event(CS_ModemEvent_t events_mask,
	...
	if (DATAPACK_writeStruct(&cmd_buf[0], (uint16_t) CSMT_MODEM_EVENT,
			(uint16_t) sizeof(CS_ModemEvent_t), (void*) &events_mask)
			== DATAPACK_OK) {
		at_status_t err;
		err = AT_sendcmd(_Adapter_Handle,
				(at_msg_t) SID_CS_SUSBCRIBE_MODEM_EVENT, &cmd_buf[0],
				&rsp_buf[0]);
		if (err == ATSTATUS_OK) {
 ...
 * @retval CS_Status_t
 */
CS_Status_t CS_register_pdn_event(CS_PDN_conf_id_t cid,
	...
	if (cid > CS_PDN_USER_CONFIG_5) {
		PRINT_ERR(
				"<Cellular_Service> only explicit PDN user config is supported (cid=%d)",
				cid)
	} else {
		if (DATAPACK_writeStruct(&cmd_buf[0], (uint16_t) CSMT_NONE,
				(uint16_t) 0U,
			...
			at_status_t err;
			err = AT_sendcmd(_Adapter_Handle,
					(at_msg_t) SID_CS_REGISTER_PDN_EVENT, &cmd_buf[0],
					&rsp_buf[0]);
			if (err == ATSTATUS_OK) {
				PRINT_DBG(
...
CS_Status_t CS_deregister_pdn_event(CS_PDN_conf_id_t cid) {
	...
	if (cid > CS_PDN_USER_CONFIG_5) {
		...
		if (DATAPACK_writeStruct(&cmd_buf[0], (uint16_t) CSMT_NONE,
			...
			at_status_t err;
			err = AT_sendcmd(_Adapter_Handle,
					(at_msg_t) SID_CS_DEREGISTER_PDN_EVENT, &cmd_buf[0],
					&rsp_buf[0]);
			if (err == ATSTATUS_OK) {
				PRINT_DBG(
...
socket_handle_t CDS_socket_create(CS_IPaddrType_t addr_type,
		CS_TransportProtocol_t protocol, CS_PDN_conf_id_t cid) {
	...
	} else if (csint_socket_create(sockhandle, addr_type, protocol, /* default local_port = 0 */
			0U, cid) != CELLULAR_OK) {
 ...
 * @retval CS_Status_t
 */
CS_Status_t CDS_socket_bind(socket_handle_t sockHandle, uint16_t local_port) {
	CS_Status_t res;
	...
	if (cs_ctxt_sockets_info[sockHandle].state != SOCKETSTATE_CREATED) {
		PRINT_ERR(
				"<Cellular_Service> socket bind allowed only after create/before connect %ld ",
				sockHandle)
		res = CELLULAR_ERROR;
...
CS_Status_t CDS_socket_set_callbacks(socket_handle_t sockHandle,
	...
	if (cs_ctxt_sockets_info[sockHandle].state == SOCKETSTATE_NOT_ALLOC) {
		PRINT_ERR("<Cellular_Service> invalid socket handle %ld (set cb)",
				sockHandle)
		retval = CELLULAR_ERROR;
	}
	...
	/*PRINT_DBG("DBG: socket remote closed callback=%p", cs_ctxt_sockets_info[sockHandle].socket_remote_close_callback)*/
		...
		cs_ctxt_sockets_info[sockHandle].socket_data_ready_callback =
				data_ready_cb;
		cs_ctxt_sockets_info[sockHandle].socket_remote_close_callback =
				remote_close_cb;
		retval = CELLULAR_OK;
	}
...
CS_Status_t CDS_socket_set_option(socket_handle_t sockHandle,
		CS_SocketOptionLevel_t opt_level, CS_SocketOptionName_t opt_name,
		void *p_opt_val) {
	CS_Status_t retval;
...
CS_Status_t CDS_socket_connect(socket_handle_t sockHandle,
	...
	retval = csint_socket_configure_remote(sockHandle, ip_addr_type,
			p_ip_addr_value, remote_port);
	if (retval == CELLULAR_OK) {
		...
		if (DATAPACK_writePtr(&cmd_buf[0], (uint16_t) CSMT_SOCKET_INFO,
				(void*) socket_infos) == DATAPACK_OK) {
			if (socket_infos->trp_connect_mode == CS_CM_COMMAND_MODE) {
				err = AT_sendcmd(_Adapter_Handle,
						(at_msg_t) SID_CS_DIAL_COMMAND, &cmd_buf[0],
						&rsp_buf[0]);
			} else {
				/* NOT SUPPORTED YET */
				err = ATSTATUS_ERROR;
 ...
 * @retval CS_Status_t
 */
CS_Status_t CDS_socket_send(socket_handle_t sockHandle, const CS_CHAR_t *p_buf,
		uint32_t length) {
	CS_Status_t retval = CELLULAR_ERROR;
	...
	if (length > DEFAULT_IP_MAX_PACKET_SIZE) {
		PRINT_ERR("<Cellular_Service> buffer size %ld exceed maximum value %d",
				length, DEFAULT_IP_MAX_PACKET_SIZE)
	}
	/* check that socket has been allocated */
	else if (cs_ctxt_sockets_info[sockHandle].state != SOCKETSTATE_CONNECTED) {
		PRINT_ERR(
				"<Cellular_Service> socket not connected (state=%d) for handle %ld (send)",
				cs_ctxt_sockets_info[sockHandle].state, sockHandle)
	} else {
		csint_socket_data_buffer_t send_data_struct;
		(void) memset((void*) &send_data_struct, 0,
		...
		if (DATAPACK_writeStruct(&cmd_buf[0],
			...
			at_status_t err;
			err = AT_sendcmd(_Adapter_Handle, (at_msg_t) SID_CS_SEND_DATA,
					&cmd_buf[0], &rsp_buf[0]);
			if (err == ATSTATUS_OK) {
...
CS_Status_t CDS_socket_sendto(socket_handle_t sockHandle,
		const CS_CHAR_t *p_buf, uint32_t length, CS_IPaddrType_t ip_addr_type,
		CS_CHAR_t *p_ip_addr_value, uint16_t remote_port) {
	CS_Status_t retval = CELLULAR_ERROR;
	at_status_t err;
	...
	/* check p_ip_addr_value size */
		...
		if (ip_addr_length > MAX_IP_ADDR_SIZE) {
			...
			(void) memcpy((void*) send_data_struct.ip_addr_value,
					(const CS_CHAR_t*) p_ip_addr_value, ip_addr_length);
	...
	if (retval == CELLULAR_ERROR) {
		PRINT_ERR(
				"<Cellular_Service> error when sending data to socket (sendto)")
	}
	return (retval);
}
 ...
 */
int32_t CDS_socket_receive(socket_handle_t sockHandle, CS_CHAR_t *p_buf,
	...
	if (max_buf_length > DEFAULT_IP_MAX_PACKET_SIZE) {
		PRINT_ERR("<Cellular_Service> buffer size %ld exceed maximum value %d",
				max_buf_length, DEFAULT_IP_MAX_PACKET_SIZE)
	}
	/* check that socket has been allocated */
	else if (cs_ctxt_sockets_info[sockHandle].state != SOCKETSTATE_CONNECTED) {
		PRINT_ERR(
				"<Cellular_Service> socket not connected (state=%d) for handle %ld (rcv)",
				cs_ctxt_sockets_info[sockHandle].state, sockHandle)
	} else {
		csint_socket_data_buffer_t receive_data_struct = { 0 };
		(void) memset((void*) &receive_data_struct, 0,
				sizeof(csint_socket_data_buffer_t));
		...
		if (DATAPACK_writeStruct(&cmd_buf[0],
			...
			at_status_t err;
			err = AT_sendcmd(_Adapter_Handle, (at_msg_t) SID_CS_RECEIVE_DATA,
					&cmd_buf[0], &rsp_buf[0]);
			if (err == ATSTATUS_OK) {
				if (DATAPACK_readStruct(&rsp_buf[0],
						(uint16_t) CSMT_SOCKET_RXDATA,
						(uint16_t) sizeof(uint32_t), &bytes_received)
						== DATAPACK_OK) {
					status = CELLULAR_OK;
				}
			}
	...
	if (status == CELLULAR_ERROR) {
		...
		PRINT_INFO("Size of data received on the socket= %ld bytes",
				bytes_received)
 ...
 * @retval Size of received data (in bytes).
 */
int32_t CDS_socket_receivefrom(socket_handle_t sockHandle, CS_CHAR_t *p_buf,
		uint32_t max_buf_length, CS_IPaddrType_t *p_ip_addr_type,
		CS_CHAR_t *p_ip_addr_value, uint16_t *p_remote_port) {
	int32_t returned_data_size;
	CS_Status_t status = CELLULAR_ERROR;
	uint32_t bytes_received = 0U;
	...
	/* check that socket has been allocated */
		...
		if (DATAPACK_writeStruct(&cmd_buf[0],
			...
			at_status_t err;
			err = AT_sendcmd(_Adapter_Handle,
					(at_msg_t) SID_CS_RECEIVE_DATA_FROM, &cmd_buf[0],
					&rsp_buf[0]);
			if (err == ATSTATUS_OK) {
...
CS_Status_t CDS_socket_close(socket_handle_t sockHandle, uint8_t force) {
	...
	if (cs_ctxt_sockets_info[sockHandle].state == SOCKETSTATE_CONNECTED) {
		...
		if (DATAPACK_writePtr(&cmd_buf[0], (uint16_t) CSMT_SOCKET_INFO,
				(void*) socket_infos) == DATAPACK_OK) {
			at_status_t err;
			err = AT_sendcmd(_Adapter_Handle, (at_msg_t) SID_CS_SOCKET_CLOSE,
					&cmd_buf[0], &rsp_buf[0]);
			if (err == ATSTATUS_OK) {
		...
		csint_socket_deallocateHandle(sockHandle);
		retval = CELLULAR_OK;
	} else if (cs_ctxt_sockets_info[sockHandle].state
			== SOCKETSTATE_NOT_ALLOC) {
		PRINT_ERR("<Cellular_Service> invalid socket handle %ld (close)",
				sockHandle)
	} else if (cs_ctxt_sockets_info[sockHandle].state
			== SOCKETSTATE_ALLOC_BUT_INVALID) {
		PRINT_INFO(
				"<Cellular_Service> invalid socket state (after modem reboot) ")
		/* deallocate socket handle and reinit socket parameters */
		csint_socket_deallocateHandle(sockHandle);
		retval = CELLULAR_OK;
	} else {
		PRINT_ERR("<Cellular_Service> invalid socket state %d (close)",
...
CS_Status_t CDS_socket_cnx_status(socket_handle_t sockHandle,
	...
	if (cs_ctxt_sockets_info[sockHandle].state != SOCKETSTATE_CONNECTED) {
		PRINT_ERR(
				"<Cellular_Service> socket not connected (state=%d) for handle %ld (status)",
				cs_ctxt_sockets_info[sockHandle].state, sockHandle)
	} else {
		/* Send socket information to ATcustom */
		...
		if (DATAPACK_writePtr(&cmd_buf[0], (uint16_t) CSMT_SOCKET_CNX_STATUS,
				(void*) &socket_cnx_infos) == DATAPACK_OK) {
			at_status_t err;
			err = AT_sendcmd(_Adapter_Handle,
					(at_msg_t) SID_CS_SOCKET_CNX_STATUS, &cmd_buf[0],
					&rsp_buf[0]);
			if (err == ATSTATUS_OK) {
...
CS_Status_t CS_suspend_data(void) {
	...
	NULL) == DATAPACK_OK) {
		at_status_t err;
		err = AT_sendcmd(_Adapter_Handle, (at_msg_t) SID_CS_DATA_SUSPEND,
				&cmd_buf[0], &rsp_buf[0]);
		if (err == ATSTATUS_OK) {
...
CS_Status_t CS_resume_data(void) {
	...
	NULL) == DATAPACK_OK) {
		at_status_t err;
		err = AT_sendcmd(_Adapter_Handle, (at_msg_t) SID_CS_DATA_RESUME,
				&cmd_buf[0], &rsp_buf[0]);
		if (err == ATSTATUS_OK) {
 ...
 * @retval CS_Status_t
 */
CS_Status_t CS_dns_request(CS_PDN_conf_id_t cid, CS_DnsReq_t *dns_req,
		CS_DnsResp_t *dns_resp) {
	/* static strucure. Use google as default primary DNS service */
	static const CS_DnsConf_t cs_default_primary_dns_addr = {
			.primary_dns_addr = "8.8.8.8" };
	...
	if (DATAPACK_writePtr(&cmd_buf[0], (uint16_t) CSMT_DNS_REQ,
			(void*) &loc_dns_req) == DATAPACK_OK) {
		at_status_t err;
		err = AT_sendcmd(_Adapter_Handle, (at_msg_t) SID_CS_DNS_REQ,
				&cmd_buf[0], &rsp_buf[0]);
		if (err == ATSTATUS_OK) {
			if (DATAPACK_readStruct(&rsp_buf[0], (uint16_t) CSMT_DNS_REQ,
					(uint16_t) sizeof(dns_resp->host_addr), dns_resp->host_addr)
					== DATAPACK_OK) {
...
CS_Status_t CDS_ping(CS_PDN_conf_id_t cid, CS_Ping_params_t *ping_params,
	...
	(void) memcpy((void*) &loc_ping_params.ping_params, ping_params,
			sizeof(CS_Ping_params_t));

	/* save the callback */
	urc_ping_rsp_callback = cs_ping_rsp_cb;

	if (DATAPACK_writeStruct(&cmd_buf[0], (uint16_t) CSMT_PING_ADDRESS,
			(uint16_t) sizeof(csint_ping_params_t), (void*) &loc_ping_params)
			== DATAPACK_OK) {
		at_status_t err;
		err = AT_sendcmd(_Adapter_Handle, (at_msg_t) SID_CS_PING_IP_ADDRESS,
				&cmd_buf[0], &rsp_buf[0]);
		if (err == ATSTATUS_OK) {
			...
			if (DATAPACK_readStruct(rsp_buf, (uint16_t) CSMT_URC_PING_RSP,
					(uint16_t) sizeof(CS_Ping_response_t), (void*) &ping_rsp)
					== DATAPACK_OK) {
 ...
 * @retval CS_Status_t
 */
CS_Status_t CS_direct_cmd(CS_direct_cmd_tx_t *direct_cmd_tx,
	...
	if (direct_cmd_tx->cmd_size <= MAX_DIRECT_CMD_SIZE) {
		...
		PRINT_INFO(
				"<Cellular_Service> Direct command command size to big (limit=%d)",
				MAX_DIRECT_CMD_SIZE)
	}
...
CS_Status_t CS_InitPowerConfig(CS_init_power_config_t *p_power_config,
	...
	(void) memset((void*) &cs_ctxt_init_power_config, 0,
			sizeof(CS_init_power_config_t));
	(void) memcpy((void*) &cs_ctxt_init_power_config, (void*) p_power_config,
			sizeof(CS_init_power_config_t));

	if (DATAPACK_writeStruct(&cmd_buf[0], (uint16_t) CSMT_INIT_POWER_CONFIG,
			(uint16_t) sizeof(CS_init_power_config_t),
			(void*) &cs_ctxt_init_power_config) == DATAPACK_OK) {
		at_status_t err;
		err = AT_sendcmd(_Adapter_Handle, (at_msg_t) SID_CS_INIT_POWER_CONFIG,
				&cmd_buf[0], &rsp_buf[0]);
		if (err == ATSTATUS_OK) {
...
CS_Status_t CS_SetPowerConfig(CS_set_power_config_t *p_power_config) {
	...
	(void) memset((void*) &cs_ctxt_set_power_config, 0,
			sizeof(CS_set_power_config_t));
	(void) memcpy((void*) &cs_ctxt_set_power_config, (void*) p_power_config,
			sizeof(CS_set_power_config_t));

	if (DATAPACK_writeStruct(&cmd_buf[0], (uint16_t) CSMT_SET_POWER_CONFIG,
			(uint16_t) sizeof(CS_set_power_config_t),
			(void*) &cs_ctxt_set_power_config) == DATAPACK_OK) {
		at_status_t err;
		err = AT_sendcmd(_Adapter_Handle, (at_msg_t) SID_CS_SET_POWER_CONFIG,
				&cmd_buf[0], &rsp_buf[0]);
		if (err == ATSTATUS_OK) {
			PRINT_DBG("<Cellular_Service> Power configuration set")
			retval = CELLULAR_OK;
...
CS_Status_t CS_SleepRequest(void) {
	...
	NULL) == DATAPACK_OK) {
		at_status_t err;
		err = AT_sendcmd(_Adapter_Handle, (at_msg_t) SID_CS_SLEEP_REQUEST,
				&cmd_buf[0], &rsp_buf[0]);
		if (err == ATSTATUS_OK) {
...
CS_Status_t CS_SleepComplete(void) {
	...
	NULL) == DATAPACK_OK) {
		at_status_t err;
		err = AT_sendcmd(_Adapter_Handle, (at_msg_t) SID_CS_SLEEP_COMPLETE,
				&cmd_buf[0], &rsp_buf[0]);
		if (err == ATSTATUS_OK) {
...
CS_Status_t CS_SleepCancel(void) {
	...
	NULL) == DATAPACK_OK) {
		at_status_t err;
		err = AT_sendcmd(_Adapter_Handle, (at_msg_t) SID_CS_SLEEP_CANCEL,
				&cmd_buf[0], &rsp_buf[0]);
		if (err == ATSTATUS_OK) {
...
CS_Status_t CS_PowerWakeup(CS_wakeup_origin_t wakeup_origin) {
	...
	if (DATAPACK_writeStruct(&cmd_buf[0], (uint16_t) CSMT_WAKEUP_ORIGIN,
			(uint16_t) sizeof(CS_wakeup_origin_t), (void*) &wakeup_origin)
			== DATAPACK_OK) {
		at_status_t err;
		err = AT_sendcmd(_Adapter_Handle, (at_msg_t) SID_CS_WAKEUP, &cmd_buf[0],
				&rsp_buf[0]);
		if (err == ATSTATUS_OK) {
...
static CS_Status_t perform_HW_reset(void) {
	...
	if (SysCtrl_reset_device(DEVTYPE_MODEM_CELLULAR) == SCSTATUS_OK) {
		if (DATAPACK_writeStruct(&cmd_buf[0], (uint16_t) CSMT_RESET,
				(uint16_t) sizeof(CS_Reset_t), (void*) &rst_type)
				== DATAPACK_OK) {
			at_status_t err;
			err = AT_sendcmd(_Adapter_Handle, (at_msg_t) SID_CS_RESET,
					&cmd_buf[0], &rsp_buf[0]);
			if (err == ATSTATUS_OK) {
...
static CS_Status_t perform_SW_reset(void) {
	...
	if (DATAPACK_writeStruct(&cmd_buf[0], (uint16_t) CSMT_RESET,
			(uint16_t) sizeof(CS_Reset_t), (void*) &rst_type) == DATAPACK_OK) {
		at_status_t err;
		err = AT_sendcmd(_Adapter_Handle, (at_msg_t) SID_CS_RESET, &cmd_buf[0],
				&rsp_buf[0]);
		if (err == ATSTATUS_OK) {
...
static void CELLULAR_urc_notif(at_buf_t *p_rsp_buf) {
	...
	if ((msgtype == (uint16_t) CSMT_URC_EPS_NETWORK_REGISTRATION_STATUS)
		...
		if (DATAPACK_readStruct(p_rsp_buf,
				...
				(uint16_t) sizeof(CS_NetworkRegState_t), (void*) &rx_state)
				== DATAPACK_OK) {
			/* if network registration status has changed, notify client */
			if (rx_state != cs_ctxt_eps_network_reg_state) {
				PRINT_DBG(
						"<Cellular_Service> EPS network registration updated: %d",
						rx_state)
				...
				PRINT_DBG(
						"<Cellular_Service> EPS network registration unchanged")
			}
		}
	}
	/* --- GPRS NETWORK REGISTRATION URC --- */
		...
		if (DATAPACK_readStruct(p_rsp_buf,
			...
			if (rx_state != cs_ctxt_gprs_network_reg_state) {
				PRINT_DBG(
						"<Cellular_Service> GPRS network registration updated: %d",
						rx_state)
				...
				PRINT_DBG(
						"<Cellular_Service> GPRS network registration unchanged")
			}
		}
	}
	/* --- CS NETWORK REGISTRATION URC --- */
		...
		if (DATAPACK_readStruct(p_rsp_buf,
			...
			if (rx_state != cs_ctxt_cs_network_reg_state) {
				PRINT_DBG(
						"<Cellular_Service> CS network registration updated: %d",
						rx_state)
				...
				PRINT_DBG(
						"<Cellular_Service> CS network registration unchanged")
			}
		}
	}
	/* --- EPS LOCATION INFORMATION URC --- */
		...
		if (DATAPACK_readStruct(p_rsp_buf,
				...
				(uint16_t) sizeof(csint_location_info_t), (void*) &rx_loc)
				== DATAPACK_OK) {
			...
			if (loc_update == CELLULAR_TRUE) {
				if (urc_eps_location_info_callback != NULL) {
					PRINT_DBG(
							"<Cellular_Service> EPS location information info updated: lac=%d, ci=%ld",
							rx_loc.lac, rx_loc.ci)
					(*urc_eps_location_info_callback)();
				}
			} else {
				PRINT_DBG(
						"<Cellular_Service> EPS location information unchanged")
			}
		}
	}
		...
		if (DATAPACK_readStruct(p_rsp_buf,
			...
			if (loc_update == CELLULAR_TRUE) {
				if (urc_gprs_location_info_callback != NULL) {
					PRINT_DBG(
							"<Cellular_Service> GPRS location information info updated: lac=%d, ci=%ld",
							rx_loc.lac, rx_loc.ci)
					(*urc_gprs_location_info_callback)();
				}
			} else {
				PRINT_DBG(
						"<Cellular_Service> GPRS location information unchanged")
			}
		}
	}
		...
		if (DATAPACK_readStruct(p_rsp_buf, (uint16_t) CSMT_URC_CS_LOCATION_INFO,
			...
			if (loc_update == CELLULAR_TRUE) {
				if (urc_cs_location_info_callback != NULL) {
					PRINT_DBG(
							"<Cellular_Service> CS location information info updated: lac=%d, ci=%ld",
							rx_loc.lac, rx_loc.ci)
					(*urc_cs_location_info_callback)();
				}
			} else {
				PRINT_DBG(
						"<Cellular_Service> CS location information unchanged")
			}
		}
	}
	/* --- SIGNAL QUALITY URC --- */
		...
		if (DATAPACK_readStruct(p_rsp_buf, (uint16_t) CSMT_URC_SIGNAL_QUALITY,
				(uint16_t) sizeof(CS_SignalQuality_t), (void*) &local_sig_qual)
				== DATAPACK_OK) {

			if (urc_signal_quality_callback != NULL) {
				PRINT_INFO(
						"<Cellular_Service> CS signal quality info updated: rssi=%d, ber=%d",
						local_sig_qual.rssi, local_sig_qual.ber)
	...
	/* --- SOCKET DATA PENDING URC --- */
		...
		if (DATAPACK_readStruct(p_rsp_buf,
				...
				(uint16_t) sizeof(socket_handle_t), (void*) &sockHandle)
				== DATAPACK_OK) {
			if (sockHandle != CS_INVALID_SOCKET_HANDLE) {
	...
	/* --- PACKET DOMAIN EVENT URC --- */
		...
		if (DATAPACK_readStruct(p_rsp_buf,
				...
				(uint16_t) sizeof(csint_PDN_event_desc_t), (void*) &pdn_event)
				== DATAPACK_OK) {
			...
			if ((pdn_event.conf_id == CS_PDN_USER_CONFIG_1)
				...
				if (urc_packet_domain_event_callback[pdn_event.conf_id] != NULL) {
					...
					(*urc_packet_domain_event_callback[pdn_event.conf_id])(
							pdn_event.conf_id, conv_pdn_event);
				}
				...
				for (uint8_t loop = 0U; loop < CS_MAX_NB_PDP_CTXT; loop++) {
					if (urc_packet_domain_event_callback[loop] != NULL) {
						CS_PDN_conf_id_t pdn_cid = convert_index_to_PDN_conf(
								loop);
	...
	/* --- MODEM EVENT URC --- */
		...
		if (DATAPACK_readStruct(p_rsp_buf, (uint16_t) CSMT_URC_MODEM_EVENT,
				(uint16_t) sizeof(CS_ModemEvent_t), (void*) &modem_events)
				== DATAPACK_OK) {
	...
	/* --- SIM STATUS EVENT URC --- */
		...
		if (DATAPACK_readStruct(p_rsp_buf, (uint16_t) CSMT_URC_SIM_EVENT,
				(uint16_t) sizeof(CS_SimEvent_status_t), (void*) &sim_status)
				== DATAPACK_OK) {
	...
	/* --- LOW POWER STATUS EVENT URC --- */
		...
		if (DATAPACK_readStruct(p_rsp_buf, (uint16_t) CSMT_URC_LP_STATUS_EVENT,
				(uint16_t) sizeof(CS_LowPower_status_t), (void*) &lp_status)
				== DATAPACK_OK) {
			PRINT_DBG("negotiated value of T3324 = %ld",
					lp_status.nwk_active_time)
			PRINT_DBG("negotiated value of T3412 = %ld",
					lp_status.nwk_periodic_TAU)
...
static CS_Status_t CELLULAR_analyze_error_report(at_buf_t *p_rsp_buf) {
	...
	if (msgtype == (uint16_t) CSMT_ERROR_REPORT) {
		...
		if (DATAPACK_readStruct(p_rsp_buf, (uint16_t) CSMT_ERROR_REPORT,
				(uint16_t) sizeof(csint_error_report_t), (void*) &error_report)
				== DATAPACK_OK) {
