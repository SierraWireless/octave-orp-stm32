...
static at_status_t process_AT_transaction(at_msg_t msg_in_id,
		at_buf_t *p_rsp_buf);
...
static at_status_t waitFromIPC(uint32_t tickstart, uint32_t cmdTimeout,
		IPC_RxMessage_t *p_msg);
static at_action_rsp_t process_answer(at_action_send_t action_send,
		uint32_t at_cmd_timeout);
...
at_status_t AT_open_channel(at_handle_t athandle) {
	...
	if (athandle != AT_HANDLE_INVALID) {
		...
		if (IPC_open(at_context.ipc_handle, at_context.ipc_device,
				at_context.ipc_mode, msgReceivedCallback, msgSentCallback,
				NULL, custom_checkEndOfMsgCallback) == IPC_OK) {
 ...
 * @retval at_status_t
 */
at_status_t AT_sendcmd(at_handle_t athandle, at_msg_t msg_in_id,
	...
	if (athandle == AT_HANDLE_INVALID) {
		...
		if (at_context.processing_cmd == 1U) {
			TRACE_ERR(
					"!!!!!!!!!!!!!!!!!! WARNING COMMAND IS UNDER PROCESS !!!!!!!!!!!!!!!!!!")
			retval = ATSTATUS_ERROR;
...
void AT_internalEvent(sysctrl_device_type_t deviceType) {
	...
	if (deviceType == DEVTYPE_MODEM_CELLULAR) {
		(void) rtosalMessageQueuePut(q_msg_IPC_received_Id,
				(uint32_t) SIG_INTERNAL_EVENT_MODEM, (uint32_t) 0U);
	}
}
          ...
          IPC_DumpRXQueue(at_context.ipc_handle, 1);
			...
			if ((at_context.action_flags & ATACTION_RSP_FRC_END) != 0U) {
				...
				at_context.action_flags &=
						~((at_action_rsp_t) ATACTION_RSP_FRC_END);
			} else if ((at_context.action_flags & ATACTION_RSP_FRC_CONTINUE)
					!= 0U) {
				action_rsp = ATACTION_RSP_FRC_CONTINUE;
				/* clean flag */
				at_context.action_flags &=
						~((at_action_rsp_t) ATACTION_RSP_FRC_CONTINUE);
			} else if ((at_context.action_flags & ATACTION_RSP_ERROR) != 0U) {
				/* clean flag */
				at_context.action_flags &=
						~((at_action_rsp_t) ATACTION_RSP_ERROR);
...
static at_status_t process_AT_transaction(at_msg_t msg_in_id,
	...
	do {
		...
		(void) memset((void*) &build_atcmd[0], 0,
				sizeof(AT_CHAR_t) * ATCMD_MAX_CMD_SIZE);
		build_atcmd_size = 0U;
		...
		if ((action_send & ATACTION_SEND_ERROR) != 0U) {
			...
			if (build_atcmd_size > 0U) {
				...
				if ((at_context.in_data_mode == AT_TRUE)
					...
					retval = sendToIPC((uint8_t*) &build_atcmd[0],
							build_atcmd_size);
					if (retval != ATSTATUS_OK) {
...
static at_action_rsp_t analyze_action_result(at_action_rsp_t val) {
	...
	at_bool_t data_mode =
			((val & ATACTION_RSP_FLAG_DATA_MODE) != 0U) ? AT_TRUE : AT_FALSE;

	/* clean DATA flag value */
	action = (at_action_rsp_t) (val
			& ~(at_action_rsp_t) ATACTION_RSP_FLAG_DATA_MODE);
...
at_status_t atcore_task_start(osPriority taskPrio, uint16_t stackSize) {
	...
	if (AT_Core_initialized != 1U) {
		...
		s_WaitAnswer_SemaphoreId = rtosalSemaphoreNew(
				(const rtosal_char_t*) "ATCORE_SEM_WAIT_ANSWER",
