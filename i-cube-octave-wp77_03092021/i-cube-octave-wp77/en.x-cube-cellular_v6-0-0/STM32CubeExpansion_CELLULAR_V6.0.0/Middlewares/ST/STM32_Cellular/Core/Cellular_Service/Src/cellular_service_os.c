...
socket_handle_t osCDS_socket_create(CS_IPaddrType_t addr_type,
	...
	socket_handle = CDS_socket_create(addr_type, protocol, cid);
	(void) rtosalMutexRelease(CellularServiceMutexHandle);
...
CS_Status_t osCDS_socket_set_callbacks(socket_handle_t sockHandle,
	...
	result = CDS_socket_set_callbacks(sockHandle, data_ready_cb, data_sent_cb,
			remote_close_cb);
...
CS_Status_t osCDS_socket_set_option(socket_handle_t sockHandle,
	...
	result = CDS_socket_set_option(sockHandle, opt_level, opt_name, p_opt_val);
 ...
 * @retval CS_Status_t
 */
CS_Status_t osCDS_socket_bind(socket_handle_t sockHandle, uint16_t local_port) {
	CS_Status_t result = CELLULAR_ERROR;
...
CS_Status_t osCDS_socket_connect(socket_handle_t sockHandle,
		CS_IPaddrType_t addr_type, CS_CHAR_t *p_ip_addr_value,
	...
	if (CST_get_state() == CST_MODEM_DATA_READY_STATE) {
		...
		result = CDS_socket_connect(sockHandle, addr_type, p_ip_addr_value,
				remote_port);
...
CS_Status_t osCDS_socket_send(socket_handle_t sockHandle,
		const CS_CHAR_t *p_buf, uint32_t length) {
	CS_Status_t result = CELLULAR_ERROR;
	...
	if (CST_get_state() == CST_MODEM_DATA_READY_STATE) {
		...
		result = CDS_socket_send(sockHandle, p_buf, length);
 ...
 */
int32_t osCDS_socket_receive(socket_handle_t sockHandle, CS_CHAR_t *p_buf,
	...
	if (CST_get_state() == CST_MODEM_DATA_READY_STATE) {
		...
		result = CDS_socket_receive(sockHandle, p_buf, max_buf_length);
...
CS_Status_t osCDS_socket_sendto(socket_handle_t sockHandle,
		const CS_CHAR_t *p_buf, uint32_t length, CS_IPaddrType_t addr_type,
...

{
	...
	if (CST_get_state() == CST_MODEM_DATA_READY_STATE) {
		...
		result = CDS_socket_sendto(sockHandle, p_buf, length, addr_type,
 ...
 * @retval Size of received data (in bytes).
 */
int32_t osCDS_socket_receivefrom(socket_handle_t sockHandle, CS_CHAR_t *p_buf,
		uint32_t max_buf_length, CS_IPaddrType_t *p_addr_type,
	...
	if (CST_get_state() == CST_MODEM_DATA_READY_STATE) {
		...
		result = CDS_socket_receivefrom(sockHandle, p_buf, max_buf_length,
				p_addr_type, p_ip_addr_value, p_remote_port);
...
CS_Status_t osCDS_socket_close(socket_handle_t sockHandle, uint8_t force) {
	...
	result = CDS_socket_close(sockHandle, force);
...
CS_Status_t osCDS_socket_cnx_status(socket_handle_t sockHandle,
	...
	result = CDS_socket_cnx_status(sockHandle, infos);
...
void osCCS_get_wait_cs_resource(void) {
	(void) rtosalMutexAcquire(CellularServiceGeneralMutexHandle,
			RTOSAL_WAIT_FOREVER);
}
 ...
 * @retval CS_Status_t
 */
CS_Status_t osCDS_dns_request(CS_PDN_conf_id_t cid, CS_DnsReq_t *dns_req,
 ...
 * @retval CS_Status_t
 */
CS_Status_t osCDS_ping(CS_PDN_conf_id_t cid, CS_Ping_params_t *ping_params,
