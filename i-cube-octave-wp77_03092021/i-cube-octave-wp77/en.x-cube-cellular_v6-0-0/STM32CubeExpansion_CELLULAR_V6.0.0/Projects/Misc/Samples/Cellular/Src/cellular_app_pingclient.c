...
static void pingclient_thread(void *p_argument) {
	...
	for (;;) {
		...
		while ((cellular_app_is_data_ready() == false)
			...
			(void) rtosalMessageQueueGet(cellular_app_pingclient.queue_id,
					&msg_queue, RTOSAL_WAIT_FOREVER);
		}

		/* Update process status */
		cellular_app_pingclient.process_status =
		...
		while ((cellular_app_is_data_ready() == true)
				&& (cellular_app_pingclient.process_status == true)) {
			if (counter == 0U) {
				/* Need to update distant ? */
				if ((cellular_app_pingclient_socket.distant.type
						!= cellular_app_pingclient_socket_change.distant_type)
						|| (cellular_app_pingclient_socket.distant.type
								== CELLULAR_APP_DISTANT_IPx_TYPE )) {
					cellular_app_distant_update(
							cellular_app_pingclient_socket_change.distant_type,
				...
				com_sockaddr_in.sin_addr.s_addr = CELLULAR_APP_GET_DISTANTIP(
						cellular_app_pingclient_socket.distant.ip);
			...
			if (counter < iteration) {
				...
				if ((cellular_app_pingclient_socket.id == COM_HANDLE_INVALID_ID)
						|| (cellular_app_pingclient_socket.state
								== CELLULAR_APP_SOCKET_INVALID)) {
					cellular_app_pingclient_socket.id = com_ping();
					if (cellular_app_pingclient_socket.id
							> COM_HANDLE_INVALID_ID) {
						cellular_app_pingclient_socket.state =
				...
				if (cellular_app_pingclient_socket.id > COM_HANDLE_INVALID_ID) {
					result = com_ping_process(cellular_app_pingclient_socket.id,
							(const com_sockaddr_t*) &com_sockaddr_in,
							(int32_t) com_sockaddr_in.sin_len,
							PINGCLIENT_SND_RCV_TIMEOUT, &ping_rsp);

					/* Is Ping ok ? */
					if ((result == COM_ERR_OK )
							&& (ping_rsp.status == COM_ERR_OK )) {
						/* Ping is OK : display the result */
						PRINT_FORCE(
								"%s: %d bytes from %d.%d.%d.%d: seq=%.2d time= %ldms ttl=%d",
						...
						if (pingclient_stat.rsp_tot
								< (0xFFFFFFFFU - ping_rsp.time)) {
						...
						if (result == COM_SOCKETS_ERR_TIMEOUT) {
							...
							PRINT_FORCE("%s: ERROR from %d.%d.%d.%d: seq=%.2d!",
									p_cellular_app_pingclient_trace,
									COM_IP4_ADDR1(&(cellular_app_pingclient_socket.distant.ip)),
									COM_IP4_ADDR2(&(cellular_app_pingclient_socket.distant.ip)),
									COM_IP4_ADDR3(&(cellular_app_pingclient_socket.distant.ip)),
					...
					PRINT_INFO(
							"%s: low-level not ready! Wait before to try again!",
							p_cellular_app_pingclient_trace)
					/* Wait to try again */
			...
			|| (is_data_ready == false)) /* Session interrupted because data is no more ready   */
			{
				if (counter > 0U) /* and at least one Ping has been send */
				{
					...
					{
						PRINT_FORCE(
								"%s: --- %d.%d.%d.%d : min/avg/max = %ld/%ld/%ld ms ok = %d/%d ---",
								p_cellular_app_pingclient_trace,
								COM_IP4_ADDR1(&(cellular_app_pingclient_socket.distant.ip)),
								COM_IP4_ADDR2(&(cellular_app_pingclient_socket.distant.ip)),
								COM_IP4_ADDR3(&(cellular_app_pingclient_socket.distant.ip)),
								COM_IP4_ADDR4(&(cellular_app_pingclient_socket.distant.ip)),
								pingclient_stat.rsp_min,
								(pingclient_stat.rsp_tot
										/ pingclient_stat.count_ok),
								pingclient_stat.rsp_max,
								pingclient_stat.count_ok, counter)
						...
						{
							PRINT_FORCE(
									"%s: --- %d.%d.%d.%d : min/avg/max = 0/0/0 ms ok = 0/%d ---",
									p_cellular_app_pingclient_trace,
									COM_IP4_ADDR1(&(cellular_app_pingclient_socket.distant.ip)),
									COM_IP4_ADDR2(&(cellular_app_pingclient_socket.distant.ip)),
									COM_IP4_ADDR3(&(cellular_app_pingclient_socket.distant.ip)),
						...
						{
							PRINT_FORCE(
									"%s: --- %d.%d.%d.%d : min/avg/max = %ld/Overrun/%ld ms ok = %d/%d ---",
									p_cellular_app_pingclient_trace,
									COM_IP4_ADDR1(&(cellular_app_pingclient_socket.distant.ip)),
									COM_IP4_ADDR2(&(cellular_app_pingclient_socket.distant.ip)),
									COM_IP4_ADDR3(&(cellular_app_pingclient_socket.distant.ip)),
					...
					if ((counter == iteration)
							&& (cellular_app_pingclient_change.process_status
									== true)) {
						PRINT_FORCE("<<< %s Completed >>>",
								p_cellular_app_pingclient_trace)
					} else {
						PRINT_FORCE("<<< %s Stopped before the end >>>",
								p_cellular_app_pingclient_trace)
					}
				}
				/* Release Ping handle */
				if (cellular_app_pingclient_socket.state
						!= CELLULAR_APP_SOCKET_INVALID) {
					cellular_app_pingclient_socket.closing = true;
					cellular_app_socket_close(CELLULAR_APP_TYPE_PINGCLIENT, 1U,
							(&cellular_app_pingclient_socket));
				}
				/* Stop the session */
		...
		if (cellular_app_pingclient_socket.state
				!= CELLULAR_APP_SOCKET_INVALID) {
			PRINT_INFO(
					"%s: Data not ready or Process stopped! Closing the socket!",
					p_cellular_app_pingclient_trace)
			cellular_app_pingclient_socket.closing = true;
			/* If data is ready try to close the socket */
			do {
				if (cellular_app_is_data_ready() == true) {
					PRINT_INFO("%s: Data ready! Closing the session!",
							p_cellular_app_pingclient_trace)
					cellular_app_socket_close(CELLULAR_APP_TYPE_PINGCLIENT, 1U,
							&cellular_app_pingclient_socket);
				} else {
					PRINT_INFO(
							"%s: Data not ready! Wait to close the socket properly!",
							p_cellular_app_pingclient_trace)
					(void) rtosalDelay(5000U);
				}
			} while (cellular_app_pingclient_socket.state
					!= CELLULAR_APP_SOCKET_INVALID);
		}
	}
}
...
bool cellular_app_pingclient_send_msg(uint32_t queue_msg) {
	...
	status = rtosalMessageQueuePut(cellular_app_pingclient.queue_id, queue_msg,
			0U);
	if (status != osOK) {
		result = false;
		PRINT_FORCE("%s: ERROR CellularApi Msg Put Type:%d Id:%d - status:%d!",
				p_cellular_app_pingclient_trace,
				GET_CELLULAR_APP_MSG_TYPE(queue_msg),
				GET_CELLULAR_APP_MSG_ID(queue_msg), status)
	}
...
bool cellular_app_pingclient_set_status(bool process_status) {
	...
	process_status_tmp = cellular_app_get_status(
			cellular_app_pingclient.process_status,
			cellular_app_pingclient_change.process_status);
	/* Only one change at a time */
	if (((process_status == true)
			&& (process_status_tmp == CELLULAR_APP_PROCESS_OFF))
			|| ((process_status == false)
					&& (process_status_tmp == CELLULAR_APP_PROCESS_ON))) {
		...
		status = rtosalMessageQueuePut(cellular_app_pingclient.queue_id,
				queue_msg, 0U);
		if (status != osOK) {
			/* Restore old value */
			cellular_app_pingclient_change.process_status = !process_status;
			PRINT_FORCE(
					"%s: ERROR SetStatus Msg Put Type:%d Id:%d - status:%d!",
					p_cellular_app_pingclient_trace,
					GET_CELLULAR_APP_MSG_TYPE(queue_msg),
					GET_CELLULAR_APP_MSG_ID(queue_msg), status)
		} else {
		...
		PRINT_FORCE("%s: Oonly one process change at a time!",
				p_cellular_app_pingclient_trace)
	}
 ...
 * @retval bool - false/true - application not updated / application update in progress
 */
bool cellular_app_pingclient_distant_change(uint8_t index,
		cellular_app_distant_type_t distant_type, uint8_t *p_distantip,
		uint32_t distantip_len) {
	bool result = false;
	cellular_app_process_status_t process_status;

	if (index == 0U) {
		process_status = cellular_app_get_status(
				cellular_app_pingclient.process_status,
				cellular_app_pingclient_change.process_status);
		result = cellular_app_distant_change(CELLULAR_APP_TYPE_PINGCLIENT,
				index, process_status, distant_type, p_distantip, distantip_len,
				&cellular_app_pingclient_socket.distant,
...
void cellular_app_pingclient_display_status(void) {
	...
	for (uint8_t i = CELLULAR_APP_DISTANT_IP1_TYPE;
			i <= CELLULAR_APP_DISTANT_IPx_TYPE ; i++) {
		/* Check the validity of the distant ip */
		if ((cellular_app_distant_ip[i][0] != 0U)
				&& (cellular_app_distant_ip[i][1] != 0U)
				&& (cellular_app_distant_ip[i][2] != 0U)
				&& (cellular_app_distant_ip[i][3] != 0U)) {
			/* IP is valid - is it the Dynamic IP ? */
			if (i != CELLULAR_APP_DISTANT_IPx_TYPE) {
				/* it is IP1 or IP2 */
				PRINT_FORCE("IP%d: %d.%d.%d.%d",
						(i - CELLULAR_APP_DISTANT_IP1_TYPE + 1U),
						cellular_app_distant_ip[i][0],
						cellular_app_distant_ip[i][1],
						cellular_app_distant_ip[i][2],
						cellular_app_distant_ip[i][3])
			} else {
				/* it is Dynamic IP */
		...
		} else /* IP1 or IP2 is invalid or Dynamic IP not yet defined */
		{
			if (i != CELLULAR_APP_DISTANT_IPx_TYPE) {
				/* IPx invalid */
				PRINT_FORCE("IP%d: NOT valid, check Ping IP parameters !",
						(i - CELLULAR_APP_DISTANT_IP1_TYPE + 1U))
			} else {
				/* Dynamic IP undefined */
				PRINT_FORCE(
						"Dynamic IP: UNDEFINED (use command 'ping ddd.ddd.ddd.ddd' to define/start it)")
			}
		}
	}
	/* Ping index status */
...
void cellular_app_pingclient_init(void) {
	...
	cellular_app_pingclient.queue_id = rtosalMessageQueueNew(NULL,
			CELLULAR_APP_QUEUE_SIZE);

	/* Change Structure Initialization */
	cellular_app_pingclient_change.process_status =
			cellular_app_pingclient.process_status;
	cellular_app_pingclient_change.process_period =
			cellular_app_pingclient.process_period;

	/* Socket Generic Initialization : state, closing, protocol, id */
	cellular_app_socket_init(&cellular_app_pingclient_socket,
			&cellular_app_pingclient_socket_change);
	...
	cellular_app_distant_update(CELLULAR_APP_DISTANT_IP1_TYPE,
			&(cellular_app_pingclient_socket.distant));

	/* NFM Initialization */
	(void) memset((void*) &(cellular_app_pingclient_socket.nfm), 0,
			sizeof(cellular_app_socket_nfm_desc_t));
	/* Statistic Initialization */
	(void) memset((void*) &(cellular_app_pingclient_socket.stat), 0,
			sizeof(cellular_app_socket_stat_desc_t));

	/* Change Structure Initialization */
	cellular_app_pingclient_socket_change.snd_buffer_len =
			cellular_app_pingclient_socket.snd_buffer_len;
	cellular_app_pingclient_socket_change.distant_type =
			cellular_app_pingclient_socket.distant.type;
...
void cellular_app_pingclient_start(void) {
	...
	/* '+1' to copy '\0' */
	(void) memcpy(thread_name, "PingClt",
			CELLULAR_APP_MIN((len + 1U), CELLULAR_APP_THREAD_NAME_MAX));

	/* Thread Creation */
	cellular_app_pingclient.thread_id = rtosalThreadNew(
			(const rtosal_char_t*) thread_name, (os_pthread) pingclient_thread,
			PINGCLIENT_THREAD_PRIO, PINGCLIENT_THREAD_STACK_SIZE, NULL);
	/* Check Creation is ok */
	if (cellular_app_pingclient.thread_id == NULL) {
		CELLULAR_APP_ERROR(
				(CELLULAR_APP_ERROR_PINGCLIENT + (int32_t)PINGCLIENT_THREAD_NUMBER),
				ERROR_FATAL)
	}
}
