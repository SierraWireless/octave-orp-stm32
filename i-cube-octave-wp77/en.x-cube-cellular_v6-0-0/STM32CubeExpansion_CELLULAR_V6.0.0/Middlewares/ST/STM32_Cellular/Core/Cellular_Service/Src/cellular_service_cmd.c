...
#define CST_CMD_MODEM_WP77           (1)
#define CST_CMD_MODEM_HL78           (0)
#define CST_CMD_MODEM_BG96           (0)
#define CST_CMD_MODEM_TYPE1SC        (0)
#define CST_CMD_USE_MODEM_CELL_GM01Q (0)
#elif defined(USE_MODEM_BG96)
#elif defined(USE_MODEM_HL78)
/* MODEM HL78 */
#define CST_CMD_USE_MODEM_CONFIG     (1)
#define CST_CMD_MODEM_WP77           (0)
#define CST_CMD_MODEM_HL78           (1)
#define CST_CMD_MODEM_BG96           (0)
#define CST_CMD_MODEM_TYPE1SC        (0)
#define CST_CMD_USE_MODEM_CELL_GM01Q (0)
#elif defined(USE_MODEM_BG96)
...
#if ((CST_CMD_MODEM_BG96 == 1) || (CST_CMD_MODEM_WP77 == 1) || (CST_CMD_MODEM_HL78 == 1))
...
/* Bit mask of scan sequences */
#define  CST_scanseq_NB1_M1  ((ATCustom_WP77_QCFGscanseq_t) 0x030202)
#define  CST_scanseq_NB1_GSM ((ATCustom_WP77_QCFGscanseq_t) 0x030101)
#define  CST_scanseq_M1_GSM  ((ATCustom_WP77_QCFGscanseq_t) 0x020101)
#define  CST_scanseq_M1_NB1  ((ATCustom_WP77_QCFGscanseq_t) 0x020303)
#define  CST_scanseq_GSM_M1  ((ATCustom_WP77_QCFGscanseq_t) 0x010202)
#define  CST_scanseq_GSM_NB1 ((ATCustom_WP77_QCFGscanseq_t) 0x010303)

#define  CST_scanseq_GSM     ((ATCustom_WP77_QCFGscanseq_t) 0x010101)
#define  CST_scanseq_M1      ((ATCustom_WP77_QCFGscanseq_t) 0x020202)
#define  CST_scanseq_NB1     ((ATCustom_WP77_QCFGscanseq_t) 0x030303)

#endif  /* (CST_CMD_MODEM_WP77 == 1)*/

#if (CST_CMD_MODEM_HL78 == 1)
/* BG96 Modem constant definition */
...
/* Bit mask of scan sequences */
#define  CST_scanseq_NB1_M1  ((ATCustom_HL78_QCFGscanseq_t) 0x030202)
#define  CST_scanseq_NB1_GSM ((ATCustom_HL78_QCFGscanseq_t) 0x030101)
#define  CST_scanseq_M1_GSM  ((ATCustom_HL78_QCFGscanseq_t) 0x020101)
#define  CST_scanseq_M1_NB1  ((ATCustom_HL78_QCFGscanseq_t) 0x020303)
#define  CST_scanseq_GSM_M1  ((ATCustom_HL78_QCFGscanseq_t) 0x010202)
#define  CST_scanseq_GSM_NB1 ((ATCustom_HL78_QCFGscanseq_t) 0x010303)

#define  CST_scanseq_GSM     ((ATCustom_HL78_QCFGscanseq_t) 0x010101)
#define  CST_scanseq_M1      ((ATCustom_HL78_QCFGscanseq_t) 0x020202)
#define  CST_scanseq_NB1     ((ATCustom_HL78_QCFGscanseq_t) 0x030303)

#endif  /* (CST_CMD_MODEM_HL78 == 1)*/
...
static uint32_t CST_CMD_get_band(const CST_band_descr_t *band_descr,
		const uint8_t *const*argv_p, uint32_t argc, uint32_t *band_result_MSB,
		uint32_t *band_result_LSB);
static void CST_CMD_display_bitmap_name(uint32_t bitmap_MSB,
...
static void CST_getSimModeStr(uint8_t state, uint8_t *const str) {
	...
	case CA_SIM_INCORRECT_PIN:
		CST_setTextToStr(
				(uint8_t*) "SIM with an incorrect PIN password provided", str);
		break;
	case CA_SIM_BUSY:
	...
	case CA_SIM_ERROR:
		CST_setTextToStr(
				(uint8_t*) "SIM error - too many error rsp during init", str);
		break;
...
static void CST_HelpCmd(void) {
	...
	PRINT_FORCE("%s targetstate [off|sim|full|modem] (set modem state)",
			CST_cmd_label)
	PRINT_FORCE(
			"%s polling [on|off]  (enable/disable periodical modem polling)",
			CST_cmd_label)
	...
	PRINT_FORCE("%s apn [on|off] (use APN ON or OFF for active sim slot)",
			CST_cmd_label)
	PRINT_FORCE(
			"%s apnconf [<apn> [<cid> [<username> <password>]]]  (update apn configuration of active sim slot)",
			CST_cmd_label)
	...
	PRINT_FORCE(
			"%s techno on [0 (GSM)|1 (GSM_COMPACT)|2 (UTRAN)|3(GSM EDGE)|4 (UTRAN HSDPA)|5 (UTRAN HSUPA)|\
                             6 (UTRAN HSDPA HSUPA)|7(E UTRAN)|8 (EC GSM IOT)|9 (E_UTRAN_NBS1)]",
			CST_cmd_label)
}
...
static cmd_status_t CST_cmd(uint8_t *cmd_line_p) {
	...
	static uint8_t *CST_ActivateName_p[] = { (uint8_t*) "Not active",
			(uint8_t*) "Active" };

	/* Sim slot type list */
	static uint8_t *CST_SimSlotName_p[3] = { ((uint8_t*) "REMOVABLE SLOT"),
			((uint8_t*) "EXTERNAL MODEM SLOT"),
			((uint8_t*) "EXTERNAL MODEM SLOT") };

	/* Modem target state list  */
	static uint8_t *CST_TargetStateName_p[] = { ((uint8_t*) "OFF"),
			((uint8_t*) "SIM_ONLY"), ((uint8_t*) "FULL"),
			((uint8_t*) "MODEM_ONLY"), };

	/* To convert "network reg mode" integer code to a string human readable */
	static const uint8_t *CST_networkRegModeToString_p[4] = {
			((uint8_t*) "Auto"), ((uint8_t*) "Manual"),
			((uint8_t*) "Deregister"), ((uint8_t*) "Manual then auto") };
	...
	static const uint8_t *CST_operatorNameFormatToString_p[10] = {
			((uint8_t*) "Long"), ((uint8_t*) "Short"), ((uint8_t*) "Numeric"),
			((uint8_t*) ""), ((uint8_t*) ""), ((uint8_t*) ""), ((uint8_t*) ""),
			((uint8_t*) ""), ((uint8_t*) ""), ((uint8_t*) "Not present") };
	/* To convert "access techno" integer code to a string human readable */
	static const uint8_t *CST_accessTechnoToString_p[10] = { ((uint8_t*) "GSM"),
			((uint8_t*) "GSM compact"), ((uint8_t*) "UTRAN"),
			((uint8_t*) "GSM edge"), ((uint8_t*) "UTRAN HSDPA"),
			((uint8_t*) "UTRAN HSUPA"), ((uint8_t*) "UTRAN HSDPA HSUPA"),
			((uint8_t*) "E-UTRAN"), ((uint8_t*) "EC GSM IOT"),
		...
		if (memcmp((CRC_CHAR_t*) cmd_p, (CRC_CHAR_t*) CST_cmd_label,
			...
			/* -- targetstate --------------------------------------------------------------------------------------------- */
				...
				if (argc == 2U) {
					...
					PRINT_FORCE("New modem target state   : %s",
							CST_TargetStateName_p[target_state.target_state])
				}
			}
			/* -- apn ----------------------------------------------------------------------------------------------------- */
			/* command apn [on|off] */
			...
			/* -- apnconf ------------------------------------------------------------------------------------------------- */
				...
				if (cmd_status == CMD_OK) {
					...
					PRINT_FORCE(
							"APN configuration values for the sim slot (%s):",
			...
			/* -- state --------------------------------------------------------------------------------------------------- */
				...
				PRINT_FORCE("Current State  : %s",
						CST_StateName[CST_get_state()])

				PRINT_FORCE("Sim Selected   : %s",
						CST_SimSlotName_p[p_my_sim_info.sim_slot_type[p_my_sim_info.sim_index]])
				for (i = 0; i < PLF_CELLULAR_SIM_SLOT_NB; i++) {
					CST_getSimModeStr((uint8_t) p_my_sim_info.sim_status[i],
							myString);
					PRINT_FORCE("Sim %s : %s",
							CST_SimSlotName_p[p_my_sim_info.sim_slot_type[i]],
							myString)
				}
				if (cst_cmd_cellular_params.nfmc_active != 0U) {
					...
					(void) dc_com_read(&dc_com_db, DC_CELLULAR_NFMC_INFO,
							(void*) &cst_cmd_nfmc_info,
							sizeof(cst_cmd_nfmc_info));
					for (i = 0U; i < CA_NFMC_VALUES_MAX_NB ; i++) {
						PRINT_FORCE("nfmc tempo %ld   : %ld", i + 1U,
								cst_cmd_nfmc_info.tempo[i])
					}
				}
			}
          ...
          PRINT_FORCE("Switch to cmd state OK")
			...
			/* -- info ---------------------------------------------------------------------------------------------------- */
				...
				(void) dc_com_read(&dc_com_db, DC_CELLULAR_INFO,
						(void*) &cst_cmd_cellular_info,
						sizeof(cst_cmd_cellular_info));
				...
				PRINT_FORCE("Modem state          : %d (%s)",
						p_my_cellular_info.modem_state, myString)
				PRINT_FORCE("Signal Quality       : %d",
						p_my_signal_info.signal_strength.raw_value)
				PRINT_FORCE("Signal level(dBm)    : %ld",
						p_my_signal_info.signal_strength.db_value)

				PRINT_FORCE("Operator name        : %s",
						cst_cmd_cellular_info.mno_name)
				PRINT_FORCE("IMEI                 : %s",
						p_my_cellular_info.imei.value)
				PRINT_FORCE("Manuf name           : %s",
						p_my_cellular_info.identity.manufacturer_id.value)
				PRINT_FORCE("Model                : %s",
						p_my_cellular_info.identity.model_id.value)
				PRINT_FORCE("Revision             : %s",
						p_my_cellular_info.identity.revision_id.value)
				PRINT_FORCE("Serial Number        : %s",
						p_my_cellular_info.identity.serial_number_id.value)
				PRINT_FORCE("ICCID                : %s",
						cst_cmd_cellular_info.iccid)
				PRINT_FORCE("IMSI                 : %s", cst_cmd_sim_info.imsi)
				PRINT_FORCE("Network register mode: %d (%s)",
						p_my_signal_info.access_techno,
			...
			/* -- config -------------------------------------------------------------------------------------------------- */
				...
				for (i = 0; i < cst_cmd_cellular_params.sim_slot_nb; i++) {
					...
					PRINT_FORCE("APN                  : \"%s\"",
							cst_cmd_cellular_params.sim_slot[i].apn)
					PRINT_FORCE("CID                  : %d",
							cst_cmd_cellular_params.sim_slot[i].cid)
					PRINT_FORCE("username             : %s",
							cst_cmd_cellular_params.sim_slot[i].username)
					PRINT_FORCE("password             : %s",
							cst_cmd_cellular_params.sim_slot[i].password)
				}

				PRINT_FORCE("Target state         : %s",
						CST_TargetStateName_p[cst_cmd_cellular_params.target_state])
				PRINT_FORCE("Attachment timeout   : %ld ms",
						cst_cmd_cellular_params.attachment_timeout)

				PRINT_FORCE("nfmc mode            : %s",
						CST_ActivateName_p[cst_cmd_cellular_params.nfmc_active])
				if (cst_cmd_cellular_params.nfmc_active != 0U) {
					/* NFMC feature active: displays list of temporisation values*/
					for (i = 0U; i < CA_NFMC_VALUES_MAX_NB ; i++) {
						PRINT_FORCE("nfmc value %ld       : %ld", i + 1U,
								cst_cmd_cellular_params.nfmc_value[i])
					}
				}
				PRINT_FORCE("Network register mode: %d = %s",
						cst_cmd_cellular_params.operator_selector.network_reg_mode,
				...
				if (cst_cmd_cellular_params.operator_selector.network_reg_mode
					...
					PRINT_FORCE("Operator name: %s\n\r",
							cst_cmd_cellular_params.operator_selector.operator_name);
				}
					...
					if ((memcmp((CRC_CHAR_t*) argv_p[1], "on",
						...
						cst_cmd_cellular_params.operator_selector.access_techno =
								cst_convert_access_techno(
										(CS_AccessTechno_t) tmpConversion);
...
static void CST_CMD_display_seq_name(uint32_t bitmap,
		const CST_seq_descr_t *bitmap_descr) {
	uint32_t i;
...
static void CST_ModemHelpCmd(void) {
	...
	PRINT_FORCE(
			"Modem configuration commands are used to modify the modem band configuration.")
	PRINT_FORCE("Setting a new configuration is performed in two steps:")
	PRINT_FORCE("\n\r");

	PRINT_FORCE(
			"- 1st step: enter the configuration parameters using the following commands:");
	PRINT_FORCE("%s config iotopmode [M1|NB1|ALL]  sets iotop mode)",
			CST_cmd_modem_label)
	PRINT_FORCE("%s config nwscanmode [GSM|LTE|AUTO]  (sets scan mode)",
			CST_cmd_modem_label)
	PRINT_FORCE(
			"%s config gsmband [900] [1800] [850] [1900] [nochange] [any]   (sets the list of GSM bands to use)",
			CST_cmd_modem_label)
	PRINT_FORCE(
			"%s config m1band [B1] [B2] [B3] [B4] [B5] [B8] [B12] [B13] [B18] [B19] [B20] [B26] [B28] [B39]",
			CST_cmd_modem_label)
	PRINT_FORCE(
			"                 [nochange] [any]  (sets the list of M1 bands to use)")
	PRINT_FORCE(
			"%s config nb1band [B1] [B2] [B3] [B4] [B5] [B8] [B12] [B13] [B18] [B19] [B20] [B26] [B28]",
			CST_cmd_modem_label)
	PRINT_FORCE(
			"                 [nochange] [any]  (sets the list of NB1 bands to use)")
	PRINT_FORCE(
			"%s config scanseq GSM_NB1_M1|GSM_M1_NB1|M1_GSM_NB1|M1_NB1_GSM|NB1_GSM_M1|NB1_M1_GSM|GSM|M1|NB1",
			CST_cmd_modem_label)
	...
	PRINT_FORCE(
			"    (Note: the result of this command displays trace of modem response)")
	...
	PRINT_FORCE("Notes:");
	PRINT_FORCE(
			"- To use these commands, it is advised to start firmware in 'Modem power on' mode");
	...
	PRINT_FORCE("Example:");
	PRINT_FORCE(
			"to reduce M1 and NB1 bands and set M1 scan first, type the following commands:");
...
static cmd_status_t CST_ModemCmd(uint8_t *cmd_line_p) {
	...
	static uint32_t cst_cmd_m1band_MSB_default = QCFGBANDCATM1_B4_MSB
			| QCFGBANDCATM1_B13_MSB;
	static uint32_t cst_cmd_m1band_LSB_default = QCFGBANDCATM1_B4_LSB
			| QCFGBANDCATM1_B13_LSB;
	...
	/* list of scan modes */
	static const uint8_t *CST_ScanmodeName_p[] = { ((uint8_t*) "AUTO"),
			((uint8_t*) "GSM"), ((uint8_t*) "LTE") };

	/* list of iotop modes */
	static const uint8_t *CST_IotopmodeName_p[] = { ((uint8_t*) "M1"),
			((uint8_t*) "NB1"), ((uint8_t*) "ALL") };
	...
	static const CST_band_descr_t CST_Nb1band[] =
			...
			{ ((uint8_t*) "B1"), QCFGBANDCATNB1_B1_MSB, QCFGBANDCATNB1_B1_LSB },
					{ ((uint8_t*) "B2"), QCFGBANDCATNB1_B2_MSB,
							QCFGBANDCATNB1_B2_LSB }, { ((uint8_t*) "B3"),
							QCFGBANDCATNB1_B3_MSB, QCFGBANDCATNB1_B3_LSB }, {
							((uint8_t*) "B4"), QCFGBANDCATNB1_B4_MSB,
							QCFGBANDCATNB1_B4_LSB }, { ((uint8_t*) "B5"),
							QCFGBANDCATNB1_B5_MSB, QCFGBANDCATNB1_B5_LSB }, {
							((uint8_t*) "B8"), QCFGBANDCATNB1_B8_MSB,
							QCFGBANDCATNB1_B8_LSB }, { ((uint8_t*) "B12"),
							QCFGBANDCATNB1_B12_MSB, QCFGBANDCATNB1_B12_LSB }, {
							((uint8_t*) "B13"), QCFGBANDCATNB1_B13_MSB,
							QCFGBANDCATNB1_B13_LSB }, { ((uint8_t*) "B18"),
							QCFGBANDCATNB1_B18_MSB, QCFGBANDCATNB1_B18_LSB }, {
							((uint8_t*) "B19"), QCFGBANDCATNB1_B19_MSB,
							QCFGBANDCATNB1_B19_LSB }, { ((uint8_t*) "B20"),
							QCFGBANDCATNB1_B20_MSB, QCFGBANDCATNB1_B20_LSB }, {
							((uint8_t*) "B26"), QCFGBANDCATNB1_B26_MSB,
							QCFGBANDCATNB1_B26_LSB }, { ((uint8_t*) "B28"),
							QCFGBANDCATNB1_B28_MSB, QCFGBANDCATNB1_B28_LSB }, {
							((uint8_t*) "nochange"),
							QCFGBANDCATNB1_NOCHANGE_MSB,
							QCFGBANDCATNB1_NOCHANGE_LSB }, { ((uint8_t*) "any"),
							QCFGBANDCATNB1_ANY_MSB, QCFGBANDCATNB1_ANY_LSB }, {
	...
	static const CST_band_descr_t CST_M1band[] =
			...
			{ ((uint8_t*) "B1"), QCFGBANDCATM1_B1_MSB, QCFGBANDCATM1_B1_LSB }, {
					((uint8_t*) "B2"), QCFGBANDCATM1_B2_MSB,
					QCFGBANDCATM1_B2_LSB }, { ((uint8_t*) "B3"),
					QCFGBANDCATM1_B3_MSB, QCFGBANDCATM1_B3_LSB }, {
					((uint8_t*) "B4"), QCFGBANDCATM1_B4_MSB,
					QCFGBANDCATM1_B4_LSB }, { ((uint8_t*) "B5"),
					QCFGBANDCATM1_B5_MSB, QCFGBANDCATM1_B5_LSB }, {
					((uint8_t*) "B8"), QCFGBANDCATM1_B8_MSB,
					QCFGBANDCATM1_B8_LSB }, { ((uint8_t*) "B12"),
					QCFGBANDCATM1_B12_MSB, QCFGBANDCATM1_B12_LSB }, {
					((uint8_t*) "B13"), QCFGBANDCATM1_B13_MSB,
					QCFGBANDCATM1_B13_LSB }, { ((uint8_t*) "B18"),
					QCFGBANDCATM1_B18_MSB, QCFGBANDCATM1_B18_LSB }, {
					((uint8_t*) "B19"), QCFGBANDCATM1_B19_MSB,
					QCFGBANDCATM1_B19_LSB }, { ((uint8_t*) "B20"),
					QCFGBANDCATM1_B20_MSB, QCFGBANDCATM1_B20_LSB }, {
					((uint8_t*) "B26"), QCFGBANDCATM1_B26_MSB,
					QCFGBANDCATM1_B26_LSB }, { ((uint8_t*) "B28"),
					QCFGBANDCATM1_B28_MSB, QCFGBANDCATM1_B28_LSB }, {
					((uint8_t*) "B39"), QCFGBANDCATM1_B39_MSB,
					QCFGBANDCATM1_B39_LSB }, { ((uint8_t*) "nochange"),
					QCFGBANDCATM1_NOCHANGE_MSB, QCFGBANDCATM1_NOCHANGE_LSB }, {
					((uint8_t*) "any"), QCFGBANDCATM1_ANY_MSB,
					QCFGBANDCATM1_ANY_LSB }, { NULL, 0, 0 } /* Mandatory: End of table */
			};

	/* list of GSM bands */
	static const CST_band_descr_t CST_GSMband[] = {
	/* name                   value_MSB      value_LSB    */
	{ ((uint8_t*) "900"), 0, QCFGBANDGSM_900, }, { ((uint8_t*) "1800"), 0,
			QCFGBANDGSM_1800 }, { ((uint8_t*) "850"), 0, QCFGBANDGSM_850 }, {
			((uint8_t*) "1900"), 0, QCFGBANDGSM_1900 }, {
			((uint8_t*) "nochange"), 0, QCFGBANDGSM_NOCHANGE }, {
			((uint8_t*) "any"), 0, QCFGBANDGSM_ANY }, { NULL, 0, 0 } /* Mandatory: End of table */
	};
	...
	{ ((uint8_t*) "GSM_M1_NB1"), QCFGSCANSEQ_GSM_M1_NB1 }, {
			...
			((uint8_t*) "GSM_NB1"), CST_scanseq_GSM_NB1 }, { ((uint8_t*) "NB1"),
			CST_scanseq_NB1 }, { ((uint8_t*) "M1"), CST_scanseq_M1 }, {
			((uint8_t*) "GSM"), CST_scanseq_GSM }, { NULL, 0 } /* Mandatory: End of table */
	};

	/* list of scan sequences name */
	static const uint8_t *CST_ScanseqName_p[CST_CMD_SCANSEQ_NUMBER] = {
			((uint8_t*) "GSM_M1_NB1"), ((uint8_t*) "GSM_NB1_M1"),
			((uint8_t*) "M1_GSM_NB1"), ((uint8_t*) "M1_NB1_GSM"),
			((uint8_t*) "NB1_GSM_M1"), ((uint8_t*) "NB1_M1_GSM"),
			((uint8_t*) "NB1_M1"), ((uint8_t*) "NB1_GSM"),
			((uint8_t*) "M1_GSM"), ((uint8_t*) "M1_NB1"), ((uint8_t*) "GSM_M1"),
			((uint8_t*) "GSM_NB1"), ((uint8_t*) "NB1"), ((uint8_t*) "M1"),
			((uint8_t*) "GSM"),
			NULL };
			...
			if (argc == 0U) {
				...
				if (argc == 1U) {
					...
					PRINT_FORCE("GSM bands : (mask=0x%lx%08lx)",
							cst_cmd_gsmband_MSB_default,
							cst_cmd_gsmband_LSB_default)
					CST_CMD_display_bitmap_name(cst_cmd_gsmband_MSB_default,
							cst_cmd_gsmband_LSB_default, CST_GSMband);
					PRINT_FORCE("M1 bands  : (mask=0x%lx%08lx)",
							cst_cmd_m1band_MSB_default,
							cst_cmd_m1band_LSB_default)
					CST_CMD_display_bitmap_name(cst_cmd_m1band_MSB_default,
							cst_cmd_m1band_LSB_default, CST_M1band);
					PRINT_FORCE("NB1 bands : (mask=0x%lx%08lx)",
							cst_cmd_nb1band_MSB_default,
							cst_cmd_nb1band_LSB_default)
					CST_CMD_display_bitmap_name(cst_cmd_nb1band_MSB_default,
							cst_cmd_nb1band_LSB_default, CST_Nb1band);

					PRINT_FORCE("Scan seq : (mask=0x%06lx)",
							cst_cmd_scanseq_default)
					...
					if (argc == 3U) {
						...
						if (memcmp((const CRC_CHAR_t*) argv_p[2], "AUTO",
							...
							PRINT_FORCE("%s %s Bad parameter: %s",
									...
									argv_p[1], argv_p[2])
							PRINT_FORCE(
									"Usage: %s config nwscanmode [GSM|LTE|AUTO]",
									CST_cmd_modem_label)
							cmd_status = CMD_SYNTAX_ERROR;
						}
					}
					...
					if (argc == 3U) {
						if (memcmp((const CRC_CHAR_t*) argv_p[2], "M1",
							...
							PRINT_FORCE("%s %s Bad parameter: %s",
									...
									argv_p[1], argv_p[2])
							PRINT_FORCE(
									"Usage: %s config iotopmode [M1|NB1|ALL]",
									CST_cmd_modem_label)
							cmd_status = CMD_SYNTAX_ERROR;
						}
					}
					...
					if (argc >= 3U) {
						...
						ret = CST_CMD_get_band(CST_GSMband,
								(const uint8_t**) argv_p, argc,
								&gsmband_value_msb, &gsmband_value_lsb);
						if (ret != 0U) {
							/* bad band argument: display help */
							cmd_status = CMD_SYNTAX_ERROR;
							PRINT_FORCE("%s Bad parameter", CST_cmd_modem_label)
							PRINT_FORCE(
									"Usage:%s config gsmband [900] [1800] [850] [1900] [nochange] [any]",
									CST_cmd_modem_label)
						} else {
					...
					if (argc >= 3U) {
						...
						ret = CST_CMD_get_band(CST_M1band,
								(const uint8_t**) argv_p, argc,
								&m1band_value_msb, &m1band_value_lsb);
						if (ret != 0U) {
							/* bad band argument: display help */
							PRINT_FORCE("%s Bad parameter", CST_cmd_modem_label)
							PRINT_FORCE(
									"Usage:%s config m1band [B1] [B2] [B3] [B4] [B5] [B8] [B12] [B13] [B18] [B19] [B20] [B26]",
									CST_cmd_modem_label)
							PRINT_FORCE(
									"                       [B28] [B39] [nchanche] [any]")
							cmd_status = CMD_SYNTAX_ERROR;
						} else {
							/* valid bands defined. Set requested bands in the current config */
					...
					if (argc >= 3U) {
						...
						ret = CST_CMD_get_band(CST_Nb1band,
								(const uint8_t**) argv_p, argc,
								&nb1band_value_msb, &nb1band_value_lsb);
						if (ret != 0U) {
							/* bad band argument: display help */
							PRINT_FORCE("%s Bad parameter", CST_cmd_modem_label)
							PRINT_FORCE(
									"Usage: modem config nb1band [B1] [B2] [B3] [B4] [B5] [B8] [B12] [B13] [B18]")
							PRINT_FORCE(
									"                            [B19] [B20] [B26] [B28] [nchanche] [any]")
							cmd_status = CMD_SYNTAX_ERROR;
						} else {
							/* valid bands defined. Set requested bands in the current config */
					...
					ret = (uint32_t) cst_at_command_handle(
							(uint8_t*) CST_CMD_Command);

					/* send iotopmode set AT command  */
					(void) sprintf((CRC_CHAR_t*) CST_CMD_Command,
							"AT+QCFG=\"iotopmode\",%ld,1",
							cst_cmd_iotopmode_default);
					ret |= (uint32_t) cst_at_command_handle(
							(uint8_t*) CST_CMD_Command);
					...
					(void) sprintf((CRC_CHAR_t*) CST_CMD_Command,
							"AT+QCFG=\"band\",%lx%08lx,%lx%08lx,%lx%08lx,1",
							cst_cmd_gsmband_MSB_default,
							cst_cmd_gsmband_LSB_default,
							cst_cmd_m1band_MSB_default,
							cst_cmd_m1band_LSB_default,
...
#endif /* defined(CST_CMD_MODEM_WP77 == 1) */
...
#endif /* defined(CST_CMD_MODEM_HL78 == 1) */
