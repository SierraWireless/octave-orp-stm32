...
/* Ashu Modified teh below Queue to 15 based on ST recommendation - Workaround */
/* Maximum elements in Message queue */
#define CST_QUEUE_SIZE (15U)
...
CS_NRS_NOT_REGISTERED_NOT_SEARCHING, CS_NRS_NOT_REGISTERED_NOT_SEARCHING,
		CS_NRS_NOT_REGISTERED_NOT_SEARCHING, 0U, /* activate_pdn_nfmc_tempo_count */
...
static void CST_pdn_event_callback(CS_PDN_conf_id_t cid,
...
#endif /* (USE_LOW_POWER == 1) */
static void CST_notif_callback(dc_com_event_id_t dc_event_id,
...
static void CST_waiting_for_signal_quality_ok_state(
...
const uint8_t *CST_StateName[CST_MAX_STATE] = { ((uint8_t*) "BOOT_STATE"),
		((uint8_t*) "MODEM_INIT_STATE"), ((uint8_t*) "MODEM_READY_STATE"),
		...
		((uint8_t*) "MODEM_REPROG_STATE"), ((uint8_t*) "MODEM_FAIL_STATE"),
		((uint8_t*) "MODEM_SIM_ONLY_STATE"), ((uint8_t*) "MODEM_RESET_STATE"),
		((uint8_t*) "MODEM_REBOOT_STATE"), ((uint8_t*) "MODEM_OFF_STATE"),
...
static void CST_pdn_event_callback(CS_PDN_conf_id_t cid,
  ...
  UNUSED(cid);
	...
	PRINT_CELLULAR_SERVICE(
			"====================================CST_pdn_event_callback (cid=%d / event=%d)\n\r",
			cid, pdn_event)
	...
	if ((dc_event_id == DC_CELLULAR_DATA_INFO)
      ...
      || (dc_event_id == DC_CELLULAR_INFO)
		...
		(void) dc_com_read(&dc_com_db, DC_CELLULAR_CONFIG,
				(void*) &cst_cellular_params, sizeof(cst_cellular_params));
...
static void CST_pdn_activate_retry_timer_callback(void *argument) {
	...
	if (cst_context.current_state == CST_MODEM_PDN_ACTIVATING_STATE) {
		...
		CST_send_message(CST_MESSAGE_CS_EVENT,
				CST_PDN_ACTIVATE_RETRY_TIMER_EVENT);
	}
}
...
static void CST_SimEventsCallback(CS_SimEvent_status_t sim_event) {
	...
	if (sim_event.event == CS_SIMEVENT_SIM_DETECT) {
		if (sim_event.param1 == CS_SIMINFOS_CARD_INSERTED) {
			...
			PRINT_CELLULAR_SERVICE(
					"*********** --> SIM CARD detect parameter unknown ****\n\r")
		}
...
static void CST_boot_power_on_only_modem_mngt(void) {
	PRINT_CELLULAR_SERVICE(
			"*********** CST_boot_modem_power_on_only_mngt ********\n\r")
...
static void CST_init_power_on_only_modem_mngt(void) {
	...
	CST_set_modem_state(&dc_com_db, CA_MODEM_STATE_POWERED_ON,
			(uint8_t*) "CA_MODEM_STATE_POWERED_ON");
...
static void CST_fota_start_event_mngt(void) {
	...
	(void) dc_com_read(&dc_com_db, DC_CELLULAR_DATA_INFO,
			(void*) &cst_cellular_data_info, sizeof(cst_cellular_data_info));
	...
	(void) dc_com_write(&dc_com_db, DC_CELLULAR_INFO,
			(void*) &cst_cellular_info, sizeof(cst_cellular_info));

	CST_set_modem_state(&dc_com_db, CA_MODEM_FOTA_INPROGRESS,
			(uint8_t*) "CA_MODEM_FOTA_INPROGRESS");
...
static void CST_modem_reboot_mngt(void) {
	...
	if (cs_status != CELLULAR_OK) {
		CST_config_fail(((uint8_t*) "CST_modem_reboot_mngt"),
				CST_MODEM_RESET_FAIL, &cst_context.reset_count,
				CST_RESET_MAX);
	} else {
		/* Data Cache -> Radio ON */
		(void) dc_com_read(&dc_com_db, DC_CELLULAR_INFO,
...
static void CST_net_register_mngt(void) {
	...
	(void) memcpy(ctxt_operator.operator_name,
			cst_cellular_params.operator_selector.operator_name,
	...
	if (cs_status == CELLULAR_OK) {
		...
		cst_context.current_EPS_NetworkRegState =
				cst_ctxt_reg_status.EPS_NetworkRegState;
		cst_context.current_GPRS_NetworkRegState =
				cst_ctxt_reg_status.GPRS_NetworkRegState;
		cst_context.current_CS_NetworkRegState =
				cst_ctxt_reg_status.CS_NetworkRegState;
		...
		CST_set_modem_state(&dc_com_db, CA_MODEM_NETWORK_REGISTERED,
				(uint8_t*) "CA_MODEM_NETWORK_REGISTERED");
		...
		CST_set_modem_state(&dc_com_db, CA_MODEM_NETWORK_SEARCHING,
				(uint8_t*) "CA_MODEM_NETWORK_SEARCHING");

		CST_config_fail(((uint8_t*) "CST_net_register_mngt"),
				CST_MODEM_REGISTER_FAIL, &cst_context.register_reset_count,
				CST_REGISTER_RESET_MAX);

		cst_context.current_EPS_NetworkRegState =
				CS_NRS_NOT_REGISTERED_SEARCHING;
		cst_context.current_GPRS_NetworkRegState =
				CS_NRS_NOT_REGISTERED_SEARCHING;
		cst_context.current_CS_NetworkRegState =
				CS_NRS_NOT_REGISTERED_SEARCHING;
...
static void CST_signal_quality_test_mngt(void) {
	...
	if (cs_status == CELLULAR_OK) {
		...
		(void) rtosalTimerStart(cst_network_status_timer_handle,
				cst_cellular_params.attachment_timeout);
...
static void CST_network_status_test_mngt(void) {
	...
	if (ret == CST_NET_REGISTERED) {
		...
		if (cs_status != CELLULAR_OK) {
			...
			(void) rtosalTimerStop(cst_network_status_timer_handle);
			PRINT_CELLULAR_SERVICE(
					"-----> BAD SIGNAL : Stop NW REG TIMEOUT TIMER\n\r")
			cst_context.register_retry_tempo_count = 0U;
		...
		CST_config_fail(((uint8_t*) "CST_network_status_test_mngt"),
				CST_MODEM_NETWORK_FAIL, &cst_context.network_reset_count,
  ...
  osCCS_get_release_cs_resource();
	...
	if (ret == CST_NET_REGISTERED) {
		...
		CST_set_modem_state(&dc_com_db, CA_MODEM_STATE_POWERED_ON,
				(uint8_t*) "CA_MODEM_STATE_POWERED_ON");
		PRINT_CELLULAR_SERVICE(
				"******** CST_pdn_event_nw_detach_mngt: osCDS_get_net_status FAIL ****\n\r")
	}
...
static void CST_network_event_mngt(void) {
    ...
    CSP_WakeupComplete();
		...
		CST_data_cache_cellular_info_set(DC_SERVICE_OFF, NULL);
		PRINT_CELLULAR_SERVICE(
				"******** CST_network_event_mngt: osCDS_get_net_status FAIL ****\n\r")
		/* unrecoverable error : fail */
		CST_config_fail(((uint8_t*) "CST_network_status_test_mngt"),
				CST_MODEM_NETWORK_FAIL, &cst_context.network_reset_count,
				CST_NETWORK_RESET_MAX);
...
static void CST_data_ready_state_network_event_mngt(void) {
	...
	if (ret == CST_NET_REGISTERED) {
		...
		CST_set_modem_state(&dc_com_db, CA_MODEM_STATE_DATAREADY,
				(uint8_t*) "CA_MODEM_STATE_DATAREADY");
...
static void CST_nw_reg_timeout_expiration_mngt(void) {
	PRINT_CELLULAR_SERVICE(
			"-----> NW REG TIMEOUT TIMER EXPIRY WE PWDN THE MODEM \n\r")
	if (cst_nfmc_context.active == true) {
		...
		PRINT_CELLULAR_SERVICE(
				"-----> CST_waiting_for_network_status NOK - retry tempo %d : %ld\n\r",
...
static void CST_attach_modem_mngt(void) {
	...
	if (cs_status != CELLULAR_OK) {
		...
		PRINT_CELLULAR_SERVICE(
				"*********** CST_attach_modem_mngt fail ********\n\r")

		CST_set_modem_state(&dc_com_db, CA_MODEM_NETWORK_SEARCHING,
				(uint8_t*) "CA_MODEM_NETWORK_SEARCHING");

		CST_config_fail(((uint8_t*) "CS_get_attach_status FAIL"),
				CST_MODEM_ATTACH_FAIL, &cst_context.attach_reset_count,
		...
		if (cst_ctxt_attach_status == CS_PS_ATTACHED) {
			...
			/* modem not attached */
			PRINT_CELLULAR_SERVICE(
					"===CST_attach_modem_mngt - NOT ATTACHED !!! ===\n\r")
...
static void CST_modem_activate_pdn_mngt(void) {
	...
	if (cs_status != CELLULAR_OK) {
		if (cst_nfmc_context.active == false) {
			...
			PRINT_CELLULAR_SERVICE(
					"-----> CST_modem_activate_pdn_mngt NOK - retry tempo  : %d\n\r",
					CST_PDN_ACTIVATE_RETRY_DELAY)
		} else {
			(void) rtosalTimerStart(cst_pdn_activate_retry_timer_handle,
					cst_nfmc_context.tempo[cst_context.activate_pdn_nfmc_tempo_count]);
			PRINT_CELLULAR_SERVICE(
					"-----> CST_modem_activate_pdn_mngt NOK - retry tempo %d : %ld\n\r",
...
static void CST_pdn_event_mngt(void) {
  ...
  CSP_WakeupComplete();
	...
	if (cst_context.pdn_status == CS_PDN_EVENT_NW_DETACH) {
		...
		PRINT_CELLULAR_SERVICE(
				"=========CST_pdn_event_mngt CS_PDN_EVENT_NW_DEACT\n\r")
		CST_set_state(CST_MODEM_REGISTERED_STATE);
		CST_send_message(CST_MESSAGE_CS_EVENT, CST_MODEM_ATTACHED_EVENT);
	} else {
		CST_set_state(CST_WAITING_FOR_NETWORK_STATUS_STATE);
...
static void CST_target_state_cmd_event_mngt(void) {
	if (cst_cellular_params.target_state == DC_TARGET_STATE_SIM_ONLY) {
		...
		(void) dc_com_read(&dc_com_db, DC_CELLULAR_SIM_INFO,
				(void*) &cst_sim_info, sizeof(cst_sim_info));
	...
	if (CST_polling_active == true) {
      ...
      if (cs_status != CELLULAR_OK)
      {
        ...
        CST_cellular_data_fail_mngt();
			...
			PRINT_CELLULAR_SERVICE(
					"Discard pooling timer, another one is already on going\n\r")
		}
	}
...
static void CST_cellular_service_task(void *argument) {
	...
	static const uint8_t *cst_event_name[CST_MAX_EVENT] = {
			((uint8_t*) "BOOT_EVENT"), ((uint8_t*) "MODEM_INIT_EVENT"),
			...
			((uint8_t*) "POLLING_TIMER_EVENT"), ((uint8_t*) "MODEM_URC_EVENT"),
			((uint8_t*) "NO_EVENT"), ((uint8_t*) "CMD_UNKWOWN_EVENT"),
			((uint8_t*) "TARGET_STATE_CMD_EVENT"),
			((uint8_t*) "APN_CONFIG_EVENT"), ((uint8_t*) "REBOOT_MODEM_EVENT"),
			((uint8_t*) "MODEM_POWER_ON_ONLY_EVENT"),
			((uint8_t*) "NETWORK_CALLBACK_EVENT"),
			((uint8_t*) "NW_REG_TIMEOUT_TIMER_EVENT"),
			((uint8_t*) "FOTA_START_EVENT"), ((uint8_t*) "FOTA_END_EVENT"),
			((uint8_t*) "FOTA_TIMEOUT_EVENT"), ((uint8_t*) "MODEM_RESET_EVENT"),
			((uint8_t*) "PPP_OPENED_EVENT"), ((uint8_t*) "PPP_CLOSED_EVENT"),
	...
	for (;;) {
		...
		if ((cst_context.current_state < CST_MAX_STATE)
			...
			PRINT_CELLULAR_SERVICE("AUTOM TASK:  %s - %s\n\r",
					CST_StateName[cst_context.current_state],
