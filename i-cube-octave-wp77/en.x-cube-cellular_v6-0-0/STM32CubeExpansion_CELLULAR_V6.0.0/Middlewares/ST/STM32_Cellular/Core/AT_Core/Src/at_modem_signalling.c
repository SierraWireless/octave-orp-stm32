...
static CS_NetworkRegState_t convert_NetworkState(uint32_t state,
		uint8_t network_type);
static CS_PDN_conf_id_t find_user_cid_with_matching_ip_addr(
		atcustom_persistent_context_t *p_persistent_ctxt,
			...
			if (element_infos->param_rank == 2U) {
				...
				if ((AT_CHAR_t*) strstr((const CRC_CHAR_t*) &line[0],
					...
					atcm_set_error_report(CSERR_SIM, p_modem_ctxt);
						...
						"SIM PIN2 REQUIRED") != NULL) {
					p_modem_ctxt->persist.sim_pin_code_ready = AT_FALSE;
					p_modem_ctxt->persist.sim_state =
							CS_SIMSTATE_SIM_PIN2_REQUIRED;
					atcm_set_error_report(CSERR_SIM, p_modem_ctxt);
				} else if ((AT_CHAR_t*) strstr((const CRC_CHAR_t*) &line[0],
						"SIM PUK2 REQUIRED") != NULL) {
					p_modem_ctxt->persist.sim_pin_code_ready = AT_FALSE;
					p_modem_ctxt->persist.sim_state =
...
at_status_t fCmdBuild_CGSN(atparser_context_t *p_atp_ctxt,
		atcustom_modem_context_t *p_modem_ctxt) {
	at_status_t retval = ATSTATUS_OK;
...
at_status_t fCmdBuild_CFUN(atparser_context_t *p_atp_ctxt,
	...
	if (p_atp_ctxt->current_atcmd.type == ATTYPE_WRITE_CMD) {
		if (p_atp_ctxt->current_SID == (at_msg_t) SID_CS_INIT_MODEM) {
			...
			const csint_modemInit_t *modemInit_struct =
					&(p_modem_ctxt->SID_ctxt.modem_init);
		...
		if (operatorSelect->mode == CS_NRM_AUTO) {
			...
			if (operatorSelect->format != CS_ONF_NOT_PRESENT) {
				if (operatorSelect->AcT_present == CELLULAR_FALSE) {
					...
					(void) sprintf(
							(CRC_CHAR_t*) p_atp_ctxt->current_atcmd.params,
							"%d,%d,\"%s\",%d", selected_mode,
							operatorSelect->format,
							operatorSelect->operator_name, operatorSelect->AcT);
				}
			} else {
				/* <oper> is not present */
...
at_status_t fCmdBuild_CREG(atparser_context_t *p_atp_ctxt,
	...
	if (p_atp_ctxt->current_atcmd.type == ATTYPE_WRITE_CMD) {
		if (p_atp_ctxt->current_SID == (at_msg_t) SID_CS_SUSBCRIBE_NET_EVENT) {
			...
			(void) sprintf((CRC_CHAR_t*) p_atp_ctxt->current_atcmd.params, "%d",
					(uint8_t) CXREG_ENABLE_NETWK_REG_LOC_URC);
...
at_status_t fCmdBuild_CGDCONT(atparser_context_t *p_atp_ctxt,
	...
	if (p_atp_ctxt->current_atcmd.type == ATTYPE_WRITE_CMD) {
		...
		if (p_modem_ctxt->persist.pdp_ctxt_infos[current_conf_id].apn_present
			...
			p_apn =
					(CS_CHAR_t*) &p_modem_ctxt->persist.pdp_ctxt_infos[current_conf_id].apn;
		} else {
...
at_status_t fCmdBuild_CGAUTH(atparser_context_t *p_atp_ctxt,
		atcustom_modem_context_t *p_modem_ctxt) {
	at_status_t retval = ATSTATUS_OK;
	...
	if (p_atp_ctxt->current_atcmd.type == ATTYPE_WRITE_CMD) {
		...
		if (p_modem_ctxt->persist.pdp_ctxt_infos[current_conf_id].pdn_conf.username[0]
				== 0U) {
			...
			/* build command */
			(void) sprintf((CRC_CHAR_t*) p_atp_ctxt->current_atcmd.params,
					"%d,%d,\"%s\",\"%s\"", modem_cid, auth_protocol,
...
at_status_t fCmdBuild_ATD(atparser_context_t *p_atp_ctxt,
		atcustom_modem_context_t *p_modem_ctxt) {
	UNUSED(p_modem_ctxt);
...
at_status_t fCmdBuild_ATZ(atparser_context_t *p_atp_ctxt,
		atcustom_modem_context_t *p_modem_ctxt) {
	UNUSED(p_modem_ctxt);
	...
	if (p_atp_ctxt->current_atcmd.type == ATTYPE_WRITE_CMD) {
		...
		if (p_modem_ctxt->SID_ctxt.set_power_config.edrx_present
			...
			if (p_modem_ctxt->SID_ctxt.set_power_config.edrx_mode
				...
				uint8_t edrx_mode;
				edrx_mode =
						(p_modem_ctxt->SID_ctxt.set_power_config.edrx_mode
								== EDRX_MODE_ENABLE_WITH_URC ) ? 2U : 1U;
...
at_action_rsp_t fRspAnalyze_Error(at_context_t *p_at_ctxt,
		atcustom_modem_context_t *p_modem_ctxt, const IPC_RxMessage_t *p_msg_in,
		at_element_info_t *element_infos) {
...
at_action_rsp_t fRspAnalyze_CmeErr(at_context_t *p_at_ctxt,
	...
	case CMD_AT_CGSN: {
		if (p_modem_ctxt->CMD_ctxt.cgsn_write_cmd_param == CGSN_SN) {
			...
			PRINT_DBG(
					"Modem Error for CGSN_IMEISV, use unitialized value, parameter ignored")
		} else if (p_modem_ctxt->CMD_ctxt.cgsn_write_cmd_param == CGSN_SVN) {
			PRINT_DBG(
					"Modem Error for CGSN_SVN, use unitialized value, parameter ignored")
		} else {
	...
	case CMD_AT_CIMI:
		...
		(void) memset((void*) &(p_modem_ctxt->SID_ctxt.device_info->u.imsi), 0,
				MAX_SIZE_IMSI);
		break;
	...
	case CMD_AT_CGMR:
		...
		(void) memset((void*) &(p_modem_ctxt->SID_ctxt.device_info->u.revision),
				0, MAX_SIZE_REV);
		break;
	...
	case CMD_AT_CNUM:
		...
		(void) memset(
				(void*) &(p_modem_ctxt->SID_ctxt.device_info->u.phone_number),
				0, MAX_SIZE_PHONE_NBR);
		break;

	case CMD_AT_GSN:
		PRINT_DBG("Modem Error for GSN, use unitialized value")
		(void) memset((void*) &(p_modem_ctxt->SID_ctxt.device_info->u.imei), 0,
				MAX_SIZE_IMEI);
		break;
	...
	case CMD_AT_CPIN:
		...
		/******Ashu Modification******/
		// As HL78 and WP77 modem returns SIM not Ready when its not yet ready to process AT commands like AT+CPIN? hence ignore and try after a tempo of 3 seconds.
					...
					if (p_modem_ctxt->CMD_ctxt.cgsn_write_cmd_param
						...
						uint16_t real_size =
								ATutil_remove_quotes(
										(const uint8_t*) &p_msg_in->buffer[element_infos->str_start_idx],
										element_infos->str_size, &tmp_array[0],
										element_infos->str_size);

						(void) memcpy(
								(void*) &(p_modem_ctxt->SID_ctxt.device_info->u.imei),
...
at_action_rsp_t fRspAnalyze_CFUN(at_context_t *p_at_ctxt,
		atcustom_modem_context_t *p_modem_ctxt, const IPC_RxMessage_t *p_msg_in,
		at_element_info_t *element_infos) {
	atparser_context_t *p_atp_ctxt = &(p_at_ctxt->parser);
	at_action_rsp_t retval = ATACTION_RSP_IGNORED;
	...
	if (p_atp_ctxt->current_atcmd.type == ATTYPE_READ_CMD) {
		START_PARAM_LOOP()
				if (element_infos->param_rank == 2U) {
					uint32_t cfun_status = ATutil_convertStringToInt(
		...
		START_PARAM_LOOP()
				if (element_infos->param_rank == 2U) {
					...
					p_modem_ctxt->SID_ctxt.read_operator_infos.optional_fields_presence |=
							CS_RSF_FORMAT_PRESENT; /* bitfield */
					switch (format) {
					case 0:
					...
					if (element_infos->str_size <= MAX_SIZE_OPERATOR_NAME) {
						...
						PRINT_DBG("+COPS: operator name = %s",
								p_modem_ctxt->SID_ctxt.read_operator_infos.operator_name)
					} else {
			...
			START_PARAM_LOOP()
					...
					if (element_infos->param_rank == 3U) {
						...
						p_modem_ctxt->persist.cs_network_state =
								convert_NetworkState(stat, CS_NETWORK_TYPE);
					...
					if (element_infos->param_rank == 4U) {
						...
						p_modem_ctxt->persist.cs_location_info.lac =
								(uint16_t) lac;
			...
			START_PARAM_LOOP()
					...
					if (element_infos->param_rank == 3U) {
						...
						p_modem_ctxt->persist.gprs_network_state =
								convert_NetworkState(stat, GPRS_NETWORK_TYPE);
					...
					if (element_infos->param_rank == 4U) {
						uint32_t lac = ATutil_extract_hex_value_from_quotes(
								&p_msg_in->buffer[element_infos->str_start_idx],
								element_infos->str_size, LAC_TAC_SIZE);
						p_modem_ctxt->persist.gprs_location_info.lac =
								(uint16_t) lac;
		...
		START_PARAM_LOOP()
				...
				if (element_infos->param_rank == 6U) {
					...
					PRINT_DBG("+CGREG URC: rac=%ld",
							ATutil_extract_hex_value_from_quotes(&p_msg_in->buffer[element_infos->str_start_idx], element_infos->str_size, RAC_SIZE))
				}
				if (element_infos->param_rank == 7U) {
					/* param traced only */
			...
			START_PARAM_LOOP()
					...
					if (element_infos->param_rank == 3U) {
						...
						p_modem_ctxt->persist.eps_network_state =
								convert_NetworkState(stat, EPS_NETWORK_TYPE);
					...
					if (element_infos->param_rank == 4U) {
						uint32_t tac = ATutil_extract_hex_value_from_quotes(
								&p_msg_in->buffer[element_infos->str_start_idx],
								element_infos->str_size, LAC_TAC_SIZE);
						p_modem_ctxt->persist.eps_location_info.lac =
								(uint16_t) tac;
					...
					} else { /* unexpecetd n value, ignore it */
					}END_PARAM_LOOP()
		}
		/* analyze parameters for +CEREG */
		else if (p_atp_ctxt->current_atcmd.type == ATTYPE_TEST_CMD) {
			...
			if (element_infos->param_rank == 2U) {
				...
				if (p_modem_ctxt->persist.urc_avail_pdn_event != AT_TRUE) {
					...
					size_t size_mini = ATC_GET_MINIMUM_SIZE(
							element_infos->str_size, MAX_CGEV_PARAM_SIZE);
					...
					while (found != NULL) {
						...
						if (0 == strcmp((CRC_CHAR_t*) found, "NW")) {
							...
							PRINT_DBG("<CLASS>")
							p_modem_ctxt->persist.pdn_event.event_type =
									CGEV_EVENT_TYPE_CLASS;
						} else if (0 == strcmp((CRC_CHAR_t*) found, "MODIFY")) {
							PRINT_DBG("<MODIFY>")
							p_modem_ctxt->persist.pdn_event.event_type =
									CGEV_EVENT_TYPE_MODIFY;
						} else {
							...
							if (p_modem_ctxt->persist.pdn_event.event_origine
								...
								if (p_modem_ctxt->persist.pdn_event.event_type
									 ...
									 */
									PRINT_ERR(
											"No parameter expected for  NW DETACH")
									...
									if (p_modem_ctxt->persist.pdn_event.event_scope
										...
										p_modem_ctxt->persist.pdn_event.conf_id =
												atcm_get_configID_for_modem_cid(
														...
														(uint8_t) cgev_cid);
										PRINT_DBG(
												"+CGEV modem cid=%ld (user conf Id =%d)",
												cgev_cid,
												p_modem_ctxt->persist.pdn_event.conf_id)
									} else {
										/* we are in the case:
					...
					PRINT_ERR(
							"an +CGEV URC still not reported, ignore this one")
					retval = ATACTION_RSP_ERROR;
				}
				...
				if ((p_modem_ctxt->persist.pdn_event.event_origine
					...
					p_modem_ctxt->persist.pdn_event.conf_id =
							find_user_cid_with_matching_ip_addr(
									&p_modem_ctxt->persist, &ip_addr_info);
		...
		START_PARAM_LOOP()
				...
				if (element_infos->param_rank == 2U) {
					...
					PRINT_DBG("+CGPADDR addr=%s",
							(AT_CHAR_t* )&ip_addr_info.ip_addr_value)
					/******Ashu Modification******/
					//To check from CGPADDR the IP address and confirm if the IP path is well created or not
					AT_CHAR_t line[32] = {0};
				...
				if (element_infos->param_rank == 2U) {
					...
					length_to_copy =
							ATC_GET_MINIMUM_SIZE(
									p_modem_ctxt->SID_ctxt.sim_generic_access.bytes_received,
									(p_modem_ctxt->SID_ctxt.sim_generic_access.data->rsp_str_size
											- 1U));
					(void) memcpy(
							(CRC_CHAR_t*) p_modem_ctxt->SID_ctxt.sim_generic_access.data->p_rsp_str,
