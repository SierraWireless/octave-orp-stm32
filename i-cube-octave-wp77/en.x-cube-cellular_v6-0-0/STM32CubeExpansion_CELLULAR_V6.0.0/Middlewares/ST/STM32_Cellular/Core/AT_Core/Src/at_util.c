	...
	if ((size > 2U) && (p_string[1] == 120U)) /* ASCII value 120 = 'x' */
	{
		...
		for (idx = 0U; idx < size; idx++) {
			...
			if ((p_string[idx] >= 48U) && (p_string[idx] <= 57U)) {
				loop++;
				conv_nbr = conv_nbr
						+ (((uint32_t) p_string[idx] - 48U)
								* ATutil_ipow(10U,
										(size - loop - nb_digit_ignored)));
			} else {
				nb_digit_ignored++;
			}
		}
	...
	if (str_size_to_convert > MAX_64BITS_STRING_SIZE) {
		...
		if (str_size_to_convert > 8U) {
			...
			*high_part_value = ATutil_convertHexaStringToInt32(
					(const uint8_t*) (p_string + nb_digit_ignored),
					high_part_size);
		} else {
			high_part_size = 0U;
		}
 ...
 * @retval size of destination string (util part).
 */
uint16_t ATutil_extract_str_from_quotes(const uint8_t *p_Src, uint16_t srcSize,
		uint8_t *p_Dst, uint16_t dstSize) {
	uint16_t src_idx;
	uint16_t dest_idx = 0U;
...
uint32_t ATutil_convert_T3412_to_seconds(uint32_t encoded_value) {
	...
	static const uint32_t AT_T3412_UNIT_COEFF[8] = { ((uint32_t) 600U), /* 0 : 10 min = 600 sec */
...
uint32_t ATutil_convert_T3324_to_seconds(uint32_t encoded_value) {
	 ...
	 */
	static const uint32_t AT_T3324_UNIT_COEFF[8] = { ((uint32_t) 2U), /* 0 : 2 sec */
