...
static uint8_t traceIF_traceComponent[DBG_CHAN_MAX_VALUE] = { 1U, /*  DBG_CHAN_MAIN              */
1U, /*  DBG_CHAN_ATCMD             */
...
static void CMD_ComponentEnableDisable(uint8_t *component, uint8_t enable) {
	...
	static uint8_t *traceIF_traceComponentName[DBG_CHAN_MAX_VALUE] = {
			(uint8_t*) "main", (uint8_t*) "atcmd",
			(uint8_t*) "cellular_service", (uint8_t*) "comlib",
			(uint8_t*) "ipc",
#if (USE_SOCKETS_TYPE == USE_SOCKETS_LWIP)
    (uint8_t *)"ppposif",
#endif /* USE_SOCKETS_TYPE == USE_SOCKETS_LWIP */
			(uint8_t*) "utilities", (uint8_t*) "error",
#if (USE_DBG_CHAN_APPLICATION == 1U)
			(uint8_t*) "app",
#endif /* USE_DBG_CHAN_APPLICATION == 1U */
			(uint8_t*) "valid" };
...
#if (USE_DBG_CHAN_APPLICATION == 1U)
	PRINT_FORCE(
			"%s enable  all|main|atcmd|cellular_service|comlib|ipc|ppposif|utilities|error|app\r\n",
			trace_cmd_label)
...
#if (USE_DBG_CHAN_APPLICATION == 1U)
	PRINT_FORCE(
			"%s disable all|main|atcmd|cellular_service|comlib|ipc|ppposif|utilities|error|app\r\n",
			trace_cmd_label)
#else /* USE_DBG_CHAN_APPLICATION == 0U or undefined */
 ...
 * @retval -
 */
void traceIF_hexPrint(dbg_channels_t chan, dbg_levels_t level, uint8_t *buff,
		uint16_t len) {
	...
	if (((tmp_len * 2U) + 1U) > MAX_HEX_PRINT_SIZE) /* +1U to ensure '\0' at the end */
	{
		...
		TRACE_PRINT(chan, level, "TR (%d/%d)\n\r",
				(MAX_HEX_PRINT_SIZE >> 1) - 2U, tmp_len)
