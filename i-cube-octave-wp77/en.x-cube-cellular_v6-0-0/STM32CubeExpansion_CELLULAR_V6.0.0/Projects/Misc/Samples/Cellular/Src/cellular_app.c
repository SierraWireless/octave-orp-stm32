...
const uint8_t *cellular_app_process_status_string[CELLULAR_APP_PROCESS_STATUS_MAX] =
		{ (uint8_t*) "Off", (uint8_t*) "On", (uint8_t*) "Stop requested",
...
const uint8_t *cellular_app_type_string[CELLULAR_APP_TYPE_MAX] = {
		(uint8_t*) "CellularApp", (uint8_t*) "Echoclt", (uint8_t*) "Ping" };

/* Private functions prototypes ----------------------------------------------*/
/* Callback for IP info: if (IP !=0U) then data is ready */
static void cellular_app_ip_info_cb(ca_event_type_t event_type,
		const cellular_ip_info_t *const p_ip_info, void *const p_callback_ctx);
	...
	if ((event_type == CA_IP_INFO_EVENT) && (p_ip_info != NULL)) {
		...
		if (p_ip_info->ip_addr.addr != 0U) {
			...
			if (cellular_app_is_data_ready() == false) {
				/* Modem is ready */
				PRINT_FORCE("%s: Modem ready to transmit data",
						p_cellular_app_trace)
			...
			if (cellular_app_is_data_ready() == true) {
				/* Modem is not ready */
				PRINT_FORCE("%s: Modem NOT ready to transmit data!",
						p_cellular_app_trace)
...
bool cellular_app_performance_start(cellular_app_type_t type, uint8_t iter_nb) {
	...
	if (cellular_app_get_performance_status() == true) {
		...
		for (uint8_t i = 0U; i < ECHOCLIENT_THREAD_NUMBER; i++) {
			...
			if (cellular_app_echoclient_get_status(i)
					!= CELLULAR_APP_PROCESS_OFF) {
				result = false;
				PRINT_FORCE(
						"%s %d: NOT fully stopped! Stop it or wait socket is closed before to retry!",
		...
		if (cellular_app_pingclient_get_status() != CELLULAR_APP_PROCESS_OFF) {
			result = false;
			PRINT_FORCE(
					"%s: NOT fully stopped! Stop it or wait session end before to retry!",
 ...
 */
cellular_app_process_status_t cellular_app_get_status(bool process_status,
		bool change_requested_process_status) {
 ...
 */
bool cellular_app_set_status(cellular_app_type_t type, uint8_t index,
		bool process_status) {
	bool result = false;
	...
	if (cellular_app_get_performance_status() == false) {
		...
		PRINT_FORCE(
				"%s: Performance test in progress! Wait its end before to retry!",
				p_cellular_app_trace)
	}
 ...
 * @retval bool - false/true - application not updated / application update in progress
 */
bool cellular_app_set_period(cellular_app_type_t type, uint8_t index,
...
void application_start(void) {
	...
	if (cellular_ip_info_cb_registration(cellular_app_ip_info_cb, (void*) NULL)
			!= CELLULAR_SUCCESS) {
