...
static void cellular_app_cmd_generic(cmd_status_t *p_cmd_status) {
	...
	} else /* At least one parameter provided */
	{
		...
		if (memcmp((CRC_CHAR_t*) p_cellular_app_cmd_argv[0], "help", len)
				== 0) {
			*p_cmd_status = CMD_OK;
...
static bool cellular_app_cmd_echoclient_generic(cmd_status_t *p_cmd_status) {
	...
	const uint8_t *trace_label =
			cellular_app_type_string[CELLULAR_APP_TYPE_ECHOCLIENT];
	...
	} else /* At least one parameter provided */
	{
		...
		if (memcmp((CRC_CHAR_t*) p_cellular_app_cmd_argv[0], "help", len)
			...
			*p_cmd_status = CMD_OK;
			...
			cellular_app_cmd_echoclient_help();
		} else if (memcmp((CRC_CHAR_t*) p_cellular_app_cmd_argv[0], "perf", len)
				== 0) {
#if (USE_LOW_POWER == 1)
      ...
      PRINT_FORCE("%s: Low power is activated! This feature is not managed!", trace_label)
			...
			*p_cmd_status = CMD_OK;
			...
			if (cellular_app_cmd_argc == 1U) {
				if (cellular_app_performance_start(CELLULAR_APP_TYPE_ECHOCLIENT,
						0U) == false) {
					*p_cmd_status = CMD_PROCESS_ERROR;
				}
			} else {
				...
				if ((atoi_res > 0)
					...
					if (cellular_app_performance_start(
							CELLULAR_APP_TYPE_ECHOCLIENT,
							(uint8_t) atoi_res) == false) {
						*p_cmd_status = CMD_PROCESS_ERROR;
					}
				} else {
					*p_cmd_status = CMD_SYNTAX_ERROR;
					/* Display a reminder about iter [min,max] */
					PRINT_FORCE("%s: Parameter iter must be [%d,%d] !",
							trace_label, ECHOCLIENT_PERF_MIN_ITER,
							ECHOCLIENT_PERF_MAX_ITER)
				}
			}
#endif /* USE_LOW_POWER == 1 */
		} else if (memcmp((CRC_CHAR_t*) p_cellular_app_cmd_argv[0], "stat", len)
				== 0) {
			*p_cmd_status = CMD_OK;
			...
			if (cellular_app_cmd_argc == 1U) {
				...
				for (uint8_t i = 0U; i < ECHOCLIENT_THREAD_NUMBER; i++) {
					...
					if (cellular_app_socket_get_stat(
							CELLULAR_APP_TYPE_ECHOCLIENT, i,
							&socket_stat) == true) {
						total_ko = socket_stat.connect.ko + socket_stat.close.ko
								+ socket_stat.send.ko + socket_stat.receive.ko;
						PRINT_FORCE("%s %d Statistics:", trace_label, (i + 1U))
						PRINT_FORCE("  o loop count:%ld ok:%ld ko:%ld",
								socket_stat.process_counter,
								socket_stat.receive.ok, total_ko)
						PRINT_FORCE(
								"  o connect: ok:%ld ko:%ld - close  : ok:%ld ko:%ld",
								socket_stat.connect.ok, socket_stat.connect.ko,
								socket_stat.close.ok, socket_stat.close.ko)
						PRINT_FORCE(
								"  o send   : ok:%ld ko:%ld - receive: ok:%ld ko:%ld",
								socket_stat.send.ok, socket_stat.send.ko,
								socket_stat.receive.ok, socket_stat.receive.ko)
					} else {
						PRINT_FORCE("%s %d: Statistics NOT found!", trace_label,
								(i + 1U))
					}
				}
				PRINT_FORCE("<<< End   %s Statistics >>>\r\n", trace_label)
			} else if (memcmp((CRC_CHAR_t*) p_cellular_app_cmd_argv[1], "reset",
					len) == 0) {
				/* cmd 'echoclient stat reset' : reset echoclient statistics */
				for (uint8_t i = 0U; i < ECHOCLIENT_THREAD_NUMBER; i++) {
					cellular_app_socket_reset_stat(CELLULAR_APP_TYPE_ECHOCLIENT,
							i);
					PRINT_FORCE("%s %d: Statistics reset", trace_label,
							(i + 1U))
				}
			} else {
				*p_cmd_status = CMD_SYNTAX_ERROR;
				PRINT_FORCE("%s: Parameter %s not supported!", trace_label,
						p_cellular_app_cmd_argv[1])
			}
		}
		/* status must be after stat because len = length of the input */
		else if (memcmp((CRC_CHAR_t*) p_cellular_app_cmd_argv[0], "status", len)
				== 0) {
...
static void cellular_app_cmd_echoclient_instance(cmd_status_t *p_cmd_status) {
	...
	*p_cmd_status = CMD_OK;
	...
	if ((*p_cellular_app_cmd_argv[argv_index] >= (uint8_t) '0')
			&& (*p_cellular_app_cmd_argv[argv_index] <= (uint8_t) '9')) {
		...
		&& (*p_cellular_app_cmd_argv[argv_index] > ((uint8_t) '0'))
				&& (*p_cellular_app_cmd_argv[argv_index]
						<= ((uint8_t) '0' + ECHOCLIENT_THREAD_NUMBER))) {
			/* instance: from 1 to ECHOCLIENT_THREAD_NUMBER
			 index   : from 0 to ECHOCLIENT_THREAD_NUMBER - 1 */
			echoclt_index = (*p_cellular_app_cmd_argv[argv_index]
					- (uint8_t) '0') - 1U;
			argv_index++; /* skip 'instance' */
		} else {
			*p_cmd_status = CMD_SYNTAX_ERROR;
			PRINT_FORCE("%s: Parameter 'instance' must be [%d, %d]!",
					trace_label, 1, ECHOCLIENT_THREAD_NUMBER)
		}
	}
	...
	if ((*p_cmd_status == CMD_OK) && (argv_index < cellular_app_cmd_argc)) {
		...
		if (memcmp((CRC_CHAR_t*) p_cellular_app_cmd_argv[argv_index], "on", len)
				== 0) {
			...
			*p_cmd_status = CMD_PROCESS_ERROR;
			...
			case CELLULAR_APP_PROCESS_OFF:
				if (cellular_app_set_status(CELLULAR_APP_TYPE_ECHOCLIENT,
						echoclt_index, true) == true) {
					PRINT_FORCE("<<< %s %d START requested...>>>", trace_label,
							(echoclt_index + 1U))
					*p_cmd_status = CMD_OK;
				} else {
					PRINT_FORCE("%s %d START rejected!", trace_label,
							(echoclt_index + 1U))
				}
				break;
			...
			case CELLULAR_APP_PROCESS_START_REQUESTED:
				PRINT_FORCE(
						"%s %d: Bad state to do this command (current state: %s) !",
						trace_label, (echoclt_index + 1U),
			...
			*p_cmd_status = CMD_PROCESS_ERROR;
			...
			case CELLULAR_APP_PROCESS_ON:
				if (cellular_app_set_status(CELLULAR_APP_TYPE_ECHOCLIENT,
						echoclt_index, false) == true) {
					PRINT_FORCE("<<< %s %d STOP requested...>>>", trace_label,
							(echoclt_index + 1U))
					*p_cmd_status = CMD_OK;
				} else {
					PRINT_FORCE("%s %d: STOP rejected!", trace_label,
							(echoclt_index + 1U))
				}
				break;
			case CELLULAR_APP_PROCESS_OFF:
			...
			*p_cmd_status = CMD_SYNTAX_ERROR;
			...
			if (cellular_app_cmd_argc == (argv_index + 1U)) {
				...
				if ((atoi_res > 0) && (atoi_res < 0xFFFF)) {
					if (cellular_app_set_snd_buffer_len(
							CELLULAR_APP_TYPE_ECHOCLIENT, echoclt_index,
							(uint16_t) atoi_res) == true) {
						*p_cmd_status = CMD_OK;
						PRINT_FORCE("%s %d: New trame size: %ldbytes requested",
								trace_label, (echoclt_index + 1U), atoi_res)
					} else {
						*p_cmd_status = CMD_PROCESS_ERROR;
						PRINT_FORCE(
								"%s %d: New trame size: %ldbytes NOT requested!",
								trace_label, (echoclt_index + 1U), atoi_res)
					}
				}
			}
			...
			*p_cmd_status = CMD_SYNTAX_ERROR;
			...
			if (cellular_app_cmd_argc == (argv_index + 1U)) {
				...
				if (atoi_res > 0) {
					if (cellular_app_set_period(CELLULAR_APP_TYPE_ECHOCLIENT,
							echoclt_index, (uint32_t) atoi_res) == true) {
						*p_cmd_status = CMD_OK;
						PRINT_FORCE(
								"%s %d: New process period: %ldms requested",
								trace_label, (echoclt_index + 1U),
								(uint32_t )atoi_res)
					} else {
						*p_cmd_status = CMD_PROCESS_ERROR;
						PRINT_FORCE(
								"%s %d: New process period: %ldms NOT requested!",
								trace_label, (echoclt_index + 1U), atoi_res)
					}
				}
			}
			...
			if (*p_cmd_status == CMD_SYNTAX_ERROR) {
				...
				PRINT_FORCE("%s: value for 'period' must be provided and > 0 !",
						trace_label)
			}
		} else if (memcmp((CRC_CHAR_t*) p_cellular_app_cmd_argv[argv_index],
				"protocol", len) == 0) {
			...
			if (cellular_app_cmd_argc == (argv_index + 1U)) {
				...
				if (memcmp((CRC_CHAR_t*) p_cellular_app_cmd_argv[argv_index],
					...
					protocol = CELLULAR_APP_SOCKET_TCP_PROTO;
						...
						(CRC_CHAR_t*) p_cellular_app_cmd_argv[argv_index],
						"UDP", len) == 0) {
          ...
          protocol = CELLULAR_APP_SOCKET_UDP_SERVICE_PROTO;
				...
				if (protocol != CELLULAR_APP_SOCKET_PROTO_MAX) {
					if (cellular_app_set_protocol(CELLULAR_APP_TYPE_ECHOCLIENT,
							echoclt_index, protocol) == true) {
						*p_cmd_status = CMD_OK;
					} else {
						*p_cmd_status = CMD_PROCESS_ERROR;
					}
        ...
        PRINT_FORCE("          TCP|UDP: mode connected")
#else
				PRINT_FORCE(
						"%s: Value for 'protocol' must be provided and [TCP|UDP|UDPSERVICE] !",
						trace_label)
				PRINT_FORCE(
						"          TCP|UDP: mode connected - UDPSERVICE: mode not-connected")
#endif /* (USE_SOCKETS_TYPE == USE_SOCKETS_MODEM) && (UDP_SERVICE_SUPPORTED == 0U) */
			}
		} else if ((memcmp((CRC_CHAR_t*) p_cellular_app_cmd_argv[argv_index],
				"server", len) == 0)
				|| (memcmp((CRC_CHAR_t*) p_cellular_app_cmd_argv[argv_index],
						"distant", len) == 0)) {
			...
			*p_cmd_status = CMD_SYNTAX_ERROR;
			...
			if (cellular_app_cmd_argc == (argv_index + 1U)) {
				...
				if (distant != CELLULAR_APP_DISTANT_TYPE_MAX) {
					if (cellular_app_echoclient_distant_change(echoclt_index,
							distant, NULL, 0U) == true) {
						*p_cmd_status = CMD_OK;
					} else {
						*p_cmd_status = CMD_PROCESS_ERROR;
					}
			...
			if (*p_cmd_status == CMD_SYNTAX_ERROR) {
				PRINT_FORCE(
						"%s: Value for server must be provided and [%s|%s|%s] !",
						trace_label,
						cellular_app_distant_string[CELLULAR_APP_DISTANT_MBED_TYPE],
						cellular_app_distant_string[CELLULAR_APP_DISTANT_UBLOX_TYPE],
						cellular_app_distant_string[CELLULAR_APP_DISTANT_LOCAL_TYPE])
			...
			*p_cmd_status = CMD_SYNTAX_ERROR;
					...
					*p_cmd_status = CMD_OK;
					...
					if (cellular_app_socket_get_stat(
						...
						TRACE_VALID("@valid@:echo%d:stat:%ld/%ld\n\r",
								(echoclt_index + 1U), socket_stat.receive.ok,
								total_ko)
					} else {
						/* Trace is specific to validation type */
						TRACE_VALID("@valid@:echo%d:stat:%d/%d - Not found\n\r",
								(echoclt_index + 1U), 0U, 0U)
					}
				}
			}
			...
			if (*p_cmd_status == CMD_SYNTAX_ERROR) {
				TRACE_VALID(
						"@valid@:echo%d:parameter for 'trace' must be provided and [stat] !\n\r",
						(echoclt_index + 1U))
			}
		} else /* cmd 'echoclient xxx' with a param xxx not supported */
		{
			*p_cmd_status = CMD_SYNTAX_ERROR;
			PRINT_FORCE("%s %s: Unrecognised parameter! Usage:", trace_label,
					p_cellular_app_cmd_argv[argv_index])
...
static void cellular_app_cmd_pingclient(cmd_status_t *p_cmd_status) {
	...
	if (cellular_app_cmd_argc == 0U) /* No parameters */
	{
		...
		*p_cmd_status = CMD_PROCESS_ERROR;
		...
		case CELLULAR_APP_PROCESS_OFF:
			/* Start Ping */
			if (cellular_app_set_status(CELLULAR_APP_TYPE_PINGCLIENT,
					pintclt_index, true) == true) {
		...
		case CELLULAR_APP_PROCESS_STOP_REQUESTED:
		case CELLULAR_APP_PROCESS_START_REQUESTED:
			PRINT_FORCE("%s: Bad state to do this command (current state: %s)!",
					trace_label,
					cellular_app_process_status_string[process_status])
			break;
		default:
	...
	} else /* At least one parameter provided */
	{
		...
		if (memcmp((CRC_CHAR_t*) p_cellular_app_cmd_argv[0], "help", len)
		...
		} else if (memcmp((CRC_CHAR_t*) p_cellular_app_cmd_argv[0], "ip1",
				crs_strlen(p_cellular_app_cmd_argv[0])) == 0) {
			/* cmd 'ping ip1': cmd allowed only if no ping in progress */
			*p_cmd_status = CMD_PROCESS_ERROR;
			if (cellular_app_pingclient_get_status()
					== CELLULAR_APP_PROCESS_OFF) {
				if (cellular_app_pingclient_distant_change(pintclt_index,
						CELLULAR_APP_DISTANT_IP1_TYPE, NULL, 0U) == true) {
					if (cellular_app_set_status(CELLULAR_APP_TYPE_PINGCLIENT,
							pintclt_index, true) == true) {
						PRINT_FORCE("<<< %s START requested...>>>", trace_label)
						*p_cmd_status = CMD_OK;
				...
				PRINT_FORCE(
						"%s: Already in progress! Stop it or wait its end before to retry!",
						trace_label)
			}
		} else if (memcmp((CRC_CHAR_t*) p_cellular_app_cmd_argv[0], "ip2",
				crs_strlen(p_cellular_app_cmd_argv[0])) == 0) {
			...
			*p_cmd_status = CMD_PROCESS_ERROR;
			if (cellular_app_pingclient_get_status()
					== CELLULAR_APP_PROCESS_OFF) {
				if (cellular_app_pingclient_distant_change(pintclt_index,
						CELLULAR_APP_DISTANT_IP2_TYPE, NULL, 0U) == true) {
					if (cellular_app_set_status(CELLULAR_APP_TYPE_PINGCLIENT,
							pintclt_index, true) == true) {
						PRINT_FORCE("<<< %s START requested...>>>", trace_label)
						*p_cmd_status = CMD_OK;
		...
		} else if (memcmp((CRC_CHAR_t*) p_cellular_app_cmd_argv[0], "status",
				len) == 0) {
			...
			if (cellular_app_pingclient_get_status()
					== CELLULAR_APP_PROCESS_OFF) {
				if (cellular_app_pingclient_distant_change(pintclt_index,
						CELLULAR_APP_DISTANT_IPx_TYPE,
						p_cellular_app_cmd_argv[0], len) == true) {
					if (cellular_app_set_status(CELLULAR_APP_TYPE_PINGCLIENT,
							pintclt_index, true) == true) {
						PRINT_FORCE("<<< %s START requested...>>>", trace_label)
						*p_cmd_status = CMD_OK;
					...
					*p_cmd_status = CMD_SYNTAX_ERROR;
					PRINT_FORCE("%s: Bad parameter or invalid IP %s!",
							p_cellular_app_cmd_argv[0], trace_label)
				}
			} else {
				*p_cmd_status = CMD_PROCESS_ERROR;
				PRINT_FORCE(
...
static cmd_status_t cellular_app_cmd_cb(uint8_t *p_cmd_line) {
	...
	if (p_cmd != NULL) {
		...
		/* Parameters parsing */
		for (cellular_app_cmd_argc = 0U;
				cellular_app_cmd_argc < CELLULAR_APP_CMD_ARG_MAX_NB;
				cellular_app_cmd_argc++) {
			p_cellular_app_cmd_argv[cellular_app_cmd_argc] = (uint8_t*) strtok(
					NULL, " \t");
...
static void cellular_app_cmd_echoclient_help(void) {
	...
	PRINT_FORCE(
			"echoclient [i] period <n> : set the process period to n (ms) for echoclient i")
	PRINT_FORCE(
			"echoclient [i] size <n>   : set buffer size to n (bytes)for echoclient i")
  ...
  PRINT_FORCE("                                    TCP|UDP: connected mode")
#else
	PRINT_FORCE(
			"echoclient [i] protocol [TCP|UDP|UDPSERVICE] : for echoclient i, set protocol to:")
	PRINT_FORCE(
			"                                               TCP|UDP: connected mode")
	PRINT_FORCE(
			"                                               UDPSERVICE: UDP not-connected mode")
#endif /* (USE_SOCKETS_TYPE == USE_SOCKETS_MODEM) && (UDP_SERVICE_SUPPORTED == 0U) */

	PRINT_FORCE(
			"echoclient [i] server [%s|%s|%s] : for echoclient i, set distant Server to %s|%s|%s",
			cellular_app_distant_string[CELLULAR_APP_DISTANT_MBED_TYPE],
			cellular_app_distant_string[CELLULAR_APP_DISTANT_UBLOX_TYPE],
			cellular_app_distant_string[CELLULAR_APP_DISTANT_LOCAL_TYPE],
			cellular_app_distant_string[CELLULAR_APP_DISTANT_MBED_TYPE],
	...
	PRINT_FORCE(
			"                        Server Name, IP and Port, Protocol, Period, Size value and state")
	PRINT_FORCE(
			"echoclient perf       : using echoclient 1, start performance snd/rcv test with default iterations nb")
	PRINT_FORCE(
			"echoclient perf <n>   : using echoclient 1, start performance snd/rcv test with only n iterations")
...
static void cellular_app_cmd_pingclient_help(void) {
	...
	PRINT_FORCE(
			"                       start a 10 pings session to IP address pointed by Ping index")
	PRINT_FORCE(
			"                       else stop the ping session and set Ping index to the next defined IP")
	PRINT_FORCE(
			"ping ip1             : if no ping in progress, set Ping index to IP1 and start a 10 pings session")
	PRINT_FORCE(
			"ping ip2             : if no ping in progress, set Ping index to IP2 and start a 10 pings session")
	PRINT_FORCE("ping ddd.ddd.ddd.ddd : if no ping in progress,")
	PRINT_FORCE(
			"                       set Dynamic IP address to ddd.ddd.ddd.ddd,r")
	PRINT_FORCE(
			"                       set Ping index to Dynamic IP and start a 10 pings session")
	PRINT_FORCE(
			"ping status          : display addresses for IP1, IP2, Dynamic IP, current Ping index and Ping state")
}
...
void cellular_app_cmd_start(void) {
	...
	CMD_Declare((uint8_t*) "cellularapp", cellular_app_cmd_cb,
			(uint8_t*) "CellularApp commands");
	/* Registration to cmd module to support echoclient cmd  */
	CMD_Declare((uint8_t*) "echoclient", cellular_app_cmd_cb,
			(uint8_t*) "EchoClient commands");
	/* Registration to cmd module to support ping cmd        */
	CMD_Declare((uint8_t*) "ping", cellular_app_cmd_cb,
			(uint8_t*) "Ping commands");
}
