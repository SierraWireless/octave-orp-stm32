...
/* Handle State */
typedef enum {
	COM_ICC_INVALID = 0, COM_ICC_CREATED, COM_ICC_WAITING_RSP
} com_icc_state_t;
...
static int32_t com_icc_use_icc_desc(int32_t handle, com_icc_state_t new_state,
		uint8_t *p_protocol);
...
static int32_t com_icc_generic_access_csim(const com_char_t *p_buf_cmd,
		int32_t len_cmd, com_char_t *p_buf_rsp, int32_t len_rsp);
#if (USE_ST33 == 1)
...
static void com_icc_datacache_cb(dc_com_event_id_t dc_event_id,
	...
	if (dc_event_id == DC_CELLULAR_SIM_INFO) {
		...
		(void) dc_com_read(&dc_com_db, DC_CELLULAR_INFO,
				(void*) &dc_cellular_info, sizeof(dc_cellular_info));
...
static int32_t com_icc_use_icc_desc(int32_t handle, com_icc_state_t new_state,
	...
	while ((i < COM_ICC_SESSION_MAX_NB) && (found == false)) {
		if ((com_icc_desc[i].handle == handle)
 ...
 */
int32_t com_icc_generic_access(int32_t icc, const com_char_t *p_buf_cmd,
		int32_t len_cmd, com_char_t *p_buf_rsp, int32_t len_rsp) {
	int32_t result;
    ...
    else if (icc_protocol == (uint8_t)COM_PROTO_NDLC)
    {
      ...
      apdu_max_length = COM_ICC_NDLC_MAX_CMD_GENERIC_ACCESS_SZ;
		...
		{
			if (icc_protocol == (uint8_t) COM_PROTO_CSIM) {
				result = com_icc_generic_access_csim(p_buf_cmd, len_cmd,
						p_buf_rsp, len_rsp);
			}
#if (USE_ST33 == 1)
      else
